document.addEventListener('DOMContentLoaded', () => {
    ...
    function doLogin(username, userId = null, email = null, session = null) {
    ...
    async function handleLogout() {
        if (currentUser) {
            const wasRemembered = currentUser.remember;
            // Rimosso il supabase.auth.signOut() globale per evitare di uccidere
            // anche le sessioni degli altri account salvati localmente.
            // Faremo solo un logout "locale" per questo utente.
            let loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
            
            if (!wasRemembered) {
                // Se non voleva essere ricordato, lo togliamo del tutto
                loggedUsers = loggedUsers.filter(u => u.email !== currentUser.email);
            } else {
                // Se voleva essere ricordato, aggiorniamo il suo stato nel selettore ma senza sessione attiva?
                // Per ora manteniamo la sessione nel selettore per "Quick Login" come richiesto dal design originale,
                // ma lo marchiamo come "non attivo" rimuovendo financeCurrentUser.
                const idx = loggedUsers.findIndex(u => u.email === currentUser.email);
                if (idx >= 0) {
                    // Stripping session to ensure "Remembered" status means "Pre-fill email" next time
            ...
            // Auto switch to another logged in user if available
            const nextUser = loggedUsers.find(u => u.session);
            if (nextUser) {
                localStorage.setItem('financeCurrentUser', JSON.stringify(nextUser));
            }
        }
        
        // Full state reset prima del ricaricamento
        monthlyBudgets = {};
        events = {};
        ...
        try {
            ...
            if (currentUser) {
                ...
                if (supabase && currentUser.id) {
                    ...
                    if (!user || user.id !== currentUser.id) {
                        if (currentUser.session) {
                            console.log("Ripristino sessione per:", currentUser.username);
                            ...
                            if (error) {
                                console.warn("Impossibile ripristinare la sessione:", error.message);
                                handleLogout();
                                return;
                            }
                        } else {
                            console.warn("Sessione Supabase non valida o assente. Reindirizzamento al login...");
                            handleLogout();
                            return;
                        }
                    }
                ...
                if (typeof setBackground === 'function') {
                    // Se non c'è uno sfondo salvato, usiamo 'bg-forest' come default
                ...
                if (typeof setCardStyle === 'function') setCardStyle(savedCardStyle);
                ...
                // Se ci sono account salvati ma nessuno è attivo, nascondi il selettore e mostra il normale login
            ...
            alert("Errore durante l'apertura dell'applicazione: " + err.message);
        }
    }
    ...
    function updateSavedAccountsUI() {
        ...
        loggedUsers.forEach(user => {
            ...
            item.addEventListener('click', async () => {
                if (user.session) {
                    doLogin(user.username, user.id, user.email, user.session);
                } else {
    ...
    function updateLoggedUsersList() {
        ...
        loggedUsers.forEach(user => {
            ...
            // Try to find profile pic in local storage for this user
            const userPic = localStorage.getItem(`${user.email}_financeProfilePic`);
            const avatarHtml = userPic 
                ? `<img src="${userPic}" class="user-avatar" style="width: 24px; height: 24px;">` 
                : `<i class="fa-solid fa-circle-user"></i>`;

            userBtn.innerHTML = `${avatarHtml} <span style="flex-grow: 1; text-align: left;">${user.username}</span> ${isCurrent ? '<i class="fa-solid fa-check" style="color: var(--success); font-size: 0.9em;"></i>' : ''}`;

            if (!isCurrent) {
                userBtn.addEventListener('click', async () => {
                    // Cliccato un account dal menu dropdown
                    if (userDropdown) userDropdown.classList.add('hidden');

                    if (user.session) {
                        // Se l'account ha una sessione attiva, fai login diretto e ricarica
                        monthlyBudgets = {};
                        events = {};
                        lastInputValues = {};
                        doLogin(user.username, user.id, user.email, user.session);
                        ...
                        // Se non c'è sessione attiva, forza la schermata di login con l'email precompilata
                        currentUser = null;
                        localStorage.removeItem('financeCurrentUser');
    ...
    if (addAccountBtnDropdown) {
        addAccountBtnDropdown.addEventListener('click', async (e) => {
            ...
            // Svuota lo stato attuale per permettere un nuovo login pulito
            // Non facciamo signOut da Supabase qui, altrimenti invalidiamo la sessione del primo account!
            
            currentUser = null;
            localStorage.removeItem('financeCurrentUser');
            
            // Pulisci le variabili globali di stato
            monthlyBudgets = {};
            events = {};
            lastInputValues = {};
            recalculateSummary();
            ...
            // Mostra la schermata auth e forza il login form (nascondendo il selettore)
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            if (events[dateStr]) {
                events[dateStr].forEach((evt, index) => {
                    ...
                    let titleText = categoryNames[evt.category] || 'Altro';
                    if (evt.isGoal) {
    ...
    function renderFamilyCalendar() {
        ...
        if (fCalendarMonthYear) fCalendarMonthYear.textContent = `${monthNames[date.getMonth()]} ${date.getFullYear()}`;
        ...
        recent.forEach(m => {
            ...
            item.innerHTML = `
                <div class="movement-info">
                    <span style="font-size: 0.9rem; font-weight: 600;">${categoryNames[m.category] || m.category}</span>
