document.addEventListener('DOMContentLoaded', () => {
    ...
    const getStorageKey = (key) => currentUser ? `${currentUser.email}_${key}` : null;
        ...
        styleOptions.forEach(option => {
            option.addEventListener('click', async () => {
                ...
                if (styleClass) {
                    ...
                    if (supabase) await syncData('card_gradient', styleClass);
                }
            });
        });
    }
    ...
    async function loadUserData() {
        ...
        // 1. Inizializzazione immediata dello stato dai dati locali (già fatto in checkAuthStatus, ma per sicurezza qui)
        ...
        // Mantieni i dati correnti se già caricati, altrimenti usa quelli locali
        if (Object.keys(monthlyBudgets).length === 0) monthlyBudgets = localBudgets;
        if (Object.keys(events).length === 0) events = localEvents;

        if (supabase && currentUser.id) {
            console.log("Tentativo di sincronizzazione dati Supabase per:", currentUser.username);
            ...
            if (error) {
                ...
                let remoteBudgets = null;
                let remoteEvents = null;
                let remoteGoals = null;

                data.forEach(item => {
                    if (item.key === 'budgets') remoteBudgets = item.value;
                    if (item.key === 'events') remoteEvents = item.value;
                    if (item.key === 'goals') remoteGoals = item.value;
                ...
                // --- LOGICA DI MERGE: Supabase vince su locale se presente ---
                if (remoteBudgets) {
                    monthlyBudgets = remoteBudgets;
                    ...
                    await syncData('budgets', localBudgets);
                }

                if (remoteEvents) {
                    events = remoteEvents;
                    localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
                } else if (Object.keys(localEvents).length > 0) {
                    await syncData('events', localEvents);
                }

                if (remoteGoals) {
                    localStorage.setItem(getStorageKey('financeGoals'), JSON.stringify(remoteGoals));
                } else {
                    const localGoals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals'))) || [];
                    if (localGoals.length > 0) await syncData('goals', localGoals);
                }

                // REFRESH UI after remote data sync
                ...
                console.log("Nessun dato trovato su Supabase. Sincronizzazione dati locali...");
                const localGoals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals'))) || [];
                if (Object.keys(localBudgets).length > 0) await syncData('budgets', localBudgets);
                if (Object.keys(localEvents).length > 0) await syncData('events', localEvents);
                if (localGoals.length > 0) await syncData('goals', localGoals);
            }
        }
    }
    ...
    function doLogin(username, userId = null, email = null, session = null) {
        currentUser = { username, id: userId, email, session };
    ...
    async function handleLogout() {
        if (currentUser) {
            // Esplicito logout da Supabase per evitare conflitti di sessione
            if (supabase) {
                await supabase.auth.signOut();
            }
            
            let loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
            // Filtra via l'account corrente usando l'email come ID unico
    ...
    async function checkAuthStatus() {
        ...
        if (currentUser) {
            ...
            const savedBg = localStorage.getItem(getStorageKey('financeBackground')) || 'bg-forest';
            setBackground(savedBg);

            const localPic = localStorage.getItem(getStorageKey('financeProfilePic'));
            updateAvatarUI(localPic); // Chiamata sempre effettuata per resettare l'icona se necessario
            loadCurrentMonthBudget();
            loadGoals();
            renderCalendar();
            initializeLastValues();
    ...
    function updateSavedAccountsUI() {
        ...
        loggedUsers.forEach(user => {
            ...
            item.addEventListener('click', async () => {
                // Svuota lo stato precedente prima del nuovo login
                monthlyBudgets = {};
                events = {};
                lastInputValues = {};
                
                doLogin(user.username, user.id, user.email, user.session);
                // Rimosso location.reload() per switch istantaneo
            });
        ...
        if (dayEventsList) {
            ...
            if (dayEvents.length === 0) {
                ...
                dayEvents.forEach((evt, index) => {
                    ...
                    const paymentStr = evt.paymentMethod ? ` [${evt.paymentMethod}]` : '';
                    let titleText = categoryNames[evt.category] || 'Altro';
                    if (evt.isGoal) {
