document.addEventListener('DOMContentLoaded', () => {
    // --- PWA Service Worker Registration ---
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', () => {
            navigator.serviceWorker.register('sw.js')
                .then(reg => console.log('Service Worker registrato con successo:', reg.scope))
                .catch(err => console.log('Errore registrazione Service Worker:', err));
        });
    }

    let deferredPrompt;
    const pwaInstallBtn = document.getElementById('pwa-install-btn');

    window.addEventListener('beforeinstallprompt', (e) => {
        console.log('Evento beforeinstallprompt intercettato! L\'app è installabile.');
        // Impedisci la visualizzazione automatica del prompt
        e.preventDefault();
        // Salva l'evento per usarlo dopo
        deferredPrompt = e;
        // Mostra il pulsante di installazione
        if (pwaInstallBtn) {
            pwaInstallBtn.classList.remove('hidden');
        }
    });

    if (pwaInstallBtn) {
        pwaInstallBtn.addEventListener('click', async (e) => {
            e.stopPropagation();
            if (!deferredPrompt) {
                alert("L'app non è ancora pronta per essere scaricata. Assicurati di:\n1. Usare un browser supportato (Chrome/Edge).\n2. Non essere in modalità Incognito.\n3. Aprire il sito tramite un server (es. Live Server) e non come file locale.");
                return;
            }
            // Mostra il prompt di installazione
            deferredPrompt.prompt();
            const { outcome } = await deferredPrompt.userChoice;
            console.log(`Risultato installazione: ${outcome}`);
            deferredPrompt = null;
            pwaInstallBtn.classList.add('hidden');
        });
    }

    // Diagnostic console message
    console.log('PWA Setup: Service Worker e Manifest configurati. In attesa di "beforeinstallprompt"...');
    if (!('serviceWorker' in navigator)) {
        console.warn('Attenzione: Il tuo browser non supporta i Service Worker. La PWA non funzionerà.');
    }
    if (window.location.protocol === 'file:') {
        console.warn('Attenzione: Le PWA non funzionano se aperte come file locale (file://). Usa un server locale.');
    }

    // --- Supabase Config ---
    const SUPABASE_URL = "https://hkcsuledqzpzqlawygsg.supabase.co";
    const SUPABASE_KEY = "sb_publishable_QCxJNslnjEIcNncc9jTDpA_YP0O56dT";
    const supabase = window.supabase ? window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY) : null;

    // --- Global Session Sync ---
    if (supabase) {
        supabase.auth.onAuthStateChange((event, session) => {
            console.log("Auth: Global Event:", event);
            if (session && session.user) {
                const email = session.user.email;
                const username = session.user.user_metadata.username || email.split('@')[0];
                
                // Aggiorna l'utente corrente SE è lo stesso che ha scatenato l'evento
                if (currentUser && currentUser.email === email) {
                    currentUser.session = session;
                    localStorage.setItem('financeCurrentUser', JSON.stringify(currentUser));
                }

                // Aggiorna sempre nell'elenco degli account salvati (se presente)
                let loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
                const idx = loggedUsers.findIndex(u => u.email === email);
                if (idx >= 0) {
                    loggedUsers[idx].session = session;
                    loggedUsers[idx].username = username; // Sincronizza anche eventuale cambio username
                    localStorage.setItem('financeLoggedUsers', JSON.stringify(loggedUsers));
                    console.log(`Auth: Sessione aggiornata per ${email} (${event})`);
                }
            }
        });
    }

    // --- Auth State ---
    const authScreen = document.getElementById('auth-screen');
    const appContainer = document.querySelector('.app-container');
    const loginForm = document.getElementById('login-form');
    const signupForm = document.getElementById('signup-form');
    const showSignupLink = document.getElementById('show-signup');
    const showLoginLink = document.getElementById('show-login');
    const loginBtn = document.getElementById('login-btn');
    const signupBtn = document.getElementById('signup-btn');
    const logoutBtn = document.getElementById('logout-btn');
    const authError = document.getElementById('auth-error');
    const currentUserNameDisplay = document.getElementById('current-user-name');
    const userAvatarImages = document.querySelectorAll('.user-avatar');
    const profilePicInput = document.getElementById('profile-pic-input');
    const avatarPreviewTrigger = document.getElementById('avatar-preview-trigger');
    const picSourceModal = document.getElementById('pic-source-modal');
    const closePicSourceModalBtn = document.getElementById('close-pic-source-modal');
    const changePicGalleryBtn = document.getElementById('change-pic-gallery-btn');
    const changePicCameraBtn = document.getElementById('change-pic-camera-btn');
    const cameraModal = document.getElementById('camera-modal');
    const closeCameraBtn = document.getElementById('close-camera-modal');
    const captureBtn = document.getElementById('capture-pic-btn');
    const cameraVideo = document.getElementById('camera-video');
    const cameraCanvas = document.getElementById('camera-canvas');
    const settingsAvatarPreview = document.getElementById('settings-avatar-preview');
    let cameraStream = null;

    // Goal Deposit Modal elements
    const depositModal = document.getElementById('deposit-modal');
    const closeDepositModalBtn = document.getElementById('close-deposit-modal');
    const depositAmountInput = document.getElementById('deposit-amount-input');
    const confirmDepositBtn = document.getElementById('confirm-deposit-btn');
    const paymentMethodBtns = document.querySelectorAll('.payment-method-btn');
    let selectedGoalId = null;
    let selectedPaymentMethod = null;

    // Chart logic
    let dashboardPieChart = null;



    let currentUser = JSON.parse(localStorage.getItem('financeCurrentUser')) || null;
    let accountContext = localStorage.getItem('financeAccountContext') || 'personal'; // 'personal' o 'family'
    let lastActiveTabId = 'tab-budget';

    // --- Elements ---
    // Inputs
    const incomeInput = document.getElementById('income');
    const rentInput = document.getElementById('rent');
    const billsInput = document.getElementById('bills');
    const autoInput = document.getElementById('auto');
    const foodInput = document.getElementById('food');
    const subsInput = document.getElementById('subs');
    const leisureInput = document.getElementById('leisure');
    const shoppingInput = document.getElementById('shopping');
    const extraInput = document.getElementById('extra');


    // --- Smart Import & Virtual Card Elements ---
    const smartImportBtn = document.getElementById('smart-import-btn');
    const smartImportModal = document.getElementById('smart-import-modal');
    const closeSmartImportBtn = document.getElementById('close-smart-import');
    const analyzeSmartImportBtn = document.getElementById('analyze-smart-import');
    const applySmartImportBtn = document.getElementById('apply-smart-import');
    const smartImportTextArea = document.getElementById('smart-import-text');
    const smartImportReview = document.getElementById('smart-import-review');
    const smartImportResultsList = document.getElementById('smart-import-results-list');
    const smartFileUpload = document.getElementById('smart-file-upload');
    const smartFileName = document.getElementById('smart-file-name');
    const fileNameSpan = document.getElementById('file-name-span');
    const allVirtualCards = document.querySelectorAll('.virtual-card');
    const styleOptions = document.querySelectorAll('.style-option');
    const settingsCardPreview = document.getElementById('settings-card-preview');

    let smartImportData = {};
    let smartImportItemsList = [];
    let fileContent = "";

    const categoryNames = {
        'it': {
            'income': 'Stipendio',
            'rent': 'Affitto / Mutuo',
            'bills': 'Bollette',
            'auto': 'Auto / Trasporti',
            'food': 'Spesa / Cibo',
            'subs': 'Abbonamenti',
            'leisure': 'Svago',
            'shopping': 'Shopping',
            'extra': 'Spese Extra',
            'other': 'Altro'
        },
        'en': {
            'income': 'Salary',
            'rent': 'Rent / Mortgage',
            'bills': 'Bills',
            'auto': 'Auto / Transport',
            'food': 'Groceries / Food',
            'subs': 'Subscriptions',
            'leisure': 'Leisure',
            'shopping': 'Shopping',
            'extra': 'Extra Expenses',
            'other': 'Other'
        },
        'es': {
            'income': 'Salario',
            'rent': 'Alquiler / Hipoteca',
            'bills': 'Facturas',
            'auto': 'Auto / Transporte',
            'food': 'Comida / Alimentos',
            'subs': 'Suscripciones',
            'leisure': 'Ocio',
            'shopping': 'Shopping',
            'extra': 'Gastos Extra',
            'other': 'Otro'
        },
        'fr': {
            'income': 'Salaire',
            'rent': 'Loyer / Hypothèque',
            'bills': 'Factures',
            'auto': 'Auto / Transport',
            'food': 'Courses / Nourriture',
            'subs': 'Abonnements',
            'leisure': 'Loisirs',
            'shopping': 'Shopping',
            'extra': 'Dépenses Extra',
            'other': 'Autre'
        }
    };

    const translations = {
        'it': {
            'app_title': 'FinanceAI - Il Tuo Assistente Finanziario',
            'app_name': 'Finanza Facile',
            'choose_account': 'Scegli un Account',
            'add_account': 'Aggiungi Account',
            'use_another_profile': 'Usa un altro profilo',
            'login': 'Accedi',
            'placeholder_email': 'Inserisci la tua email',
            'placeholder_password': 'Inserisci la tua password',
            'placeholder_new_password': 'Scegli una password',
            'placeholder_username': 'Scegli un username',
            'remember_me': 'Ricorda Account su questo dispositivo',
            'login_button': 'Accedi ora',
            'no_account': 'Non hai un account?',
            'signup_link': 'Registrati',
            'cancel': 'Annulla',
            'signup': 'Registrati',
            'min_chars': 'Minimo 6 caratteri',
            'create_account': 'Crea Account',
            'have_account': 'Hai già un account?',
            'login_link': 'Accedi',
            'current_accounts': 'Account Attuali',
            'settings': 'Impostazioni',
            'logout': 'Esci',
            'download_app': 'Scarica App',
            'appearance': 'Aspetto',
            'nav_home': 'Home',
            'nav_budget': 'Budget',
            'nav_goals': 'Obiettivi',
            'nav_family': 'Famiglia',
            'nav_stats': 'Stats',
            'nav_settings': 'Impostazioni',
            'welcome_title': 'Benvenuto nel tuo Centro Finanziario',
            'welcome_desc': 'Finanza Facile è la tua piattaforma tutto-in-uno per il controllo finanziario completo.',
            'spending_breakdown': 'Ripartizione Spese Attuali',
            'monthly_status': 'Stato Mensile',
            'guide_budget_title': 'Budget & IA',
            'guide_budget_desc': 'Pianifica reddito e spese con l\'aiuto del nostro algoritmo intelligente per massimizzare il tuo risparmio ogni mese.',
            'guide_goals_title': 'Goals & Calendario',
            'guide_goals_desc': 'Sincronizza i tuoi obiettivi di risparmio con il calendario dinamico e traccia ogni deposito in tempo reale.',
            'guide_stats_title': 'Analisi & Futuro',
            'guide_stats_desc': 'Visualizza graficamente la crescita dei tuoi risparmi e scopri come raggiungere i tuoi traguardi finanziari più velocemente.',
            'ready_start': 'Pronto per iniziare?',
            'ready_desc': 'Dirigiti alla sezione Budget & Calendario per impostare le tue finanze di questo mese.',
            'go_to_budget': 'Vai al Budget',
            'monthly_budget': 'Il Tuo Budget Mensile',
            'download_csv_demo': 'Scarica CSV Demo',
            'smart_import': 'Smart Import',
            'net_monthly_income': 'Reddito Mensile Netto (€)',
            'placeholder_income': 'Es. 2000',
            'transfer_to_family': 'Trasferisci alla Famiglia',
            'transfer_desc': 'Preleva dal tuo conto personale e aggiungi al budget comune.',
            'transfer': 'Trasferisci',
            'fixed_monthly_expenses': 'Spese Mensili Fisse',
            'cat_rent': 'Affitto / Mutuo',
            'cat_bills': 'Bollette',
            'cat_auto': 'Auto / Trasporti',
            'cat_food': 'Spesa / Cibo',
            'cat_subs': 'Abbonamenti',
            'cat_leisure': 'Svago',
            'cat_shopping': 'Shopping',
            'cat_extra': 'Extra',
            'cat_income': 'Stipendio',
            'summary': 'Resoconto',
            'income': 'Entrate',
            'expenses': 'Uscite',
            'available_balance': 'Saldo Disponibile (Rimanenza)',
            'user': 'Utente',
            'saving_tips': 'Suggerimenti per Risparmiare',
            'mon': 'Lun', 'tue': 'Mar', 'wed': 'Mer', 'thu': 'Gio', 'fri': 'Ven', 'sat': 'Sab', 'sun': 'Dom',
            'savings_goals': 'Obiettivi di Risparmio',
            'new_goal': 'Nuovo Obiettivo',
            'goal_name': 'Nome Obiettivo',
            'placeholder_goal_name': 'es. Vacanza, Nuova Auto',
            'total_cost': 'Costo Totale (€)',
            'placeholder_0': '0',
            'target_time': 'Tempo Target (mesi)',
            'monthly_deposit': 'Deposito Mensile (€)',
            'placeholder_100': 'es. 100',
            'add': 'Aggiungi',
            'yearly_stats': 'Statistiche Annuali',
            'total_yearly_salary': 'Stipendio Totale Annuo',
            'total_yearly_savings': 'Risparmio Totale Annuo',
            'chart_click_desc': 'Clicca su una fetta del grafico a torta per visualizzare qui i dettagli della categoria di spesa.',
            'monthly_savings_year': 'Risparmi Mensili dell\'Anno',
            'account_settings': 'Impostazioni Account',
            'user_profile': 'Profilo Utente',
            'profile_desc': 'Modifica le informazioni del tuo account. Queste modifiche saranno visibili in tutte le sezioni dell\'app.',
            'change_profile_pic': 'Modifica la tua foto profilo',
            'email_addr': 'Indirizzo Email (Non modificabile)',
            'placeholder_loading': 'Caricamento...',
            'username': 'Username',
            'save_changes': 'Salva Modifiche',
            'card_style': 'Stile Carta',
            'card_style_desc': 'Personalizza l\'aspetto della tua carta virtuale nel dashboard.',
            'preview': 'Anteprima',
            'username_caps': 'USERNAME',
            'bg_customization': 'Personalizzazione Sfondo',
            'bg_customization_desc': 'Scegli un tema esclusivo per il tuo spazio finanziario.',
            'language': 'Lingua',
            'language_desc': 'Scegli la lingua dell\'applicazione.',
            'connected_to_family': 'Sei collegato alla tua Famiglia!',
            'family_sync_desc': 'Sincronizza il budget, gli obiettivi e il calendario con i membri del tuo gruppo familiare.',
            'loading': 'Caricamento...',
            'no_family_prompt': 'Non fai ancora parte di un gruppo familiare. Crea un nuovo gruppo o chiedi ad un familiare di invitarti tramite la tua email.',
            'create_family_group': 'Crea Gruppo Familiare',
            'family_budget': 'Budget Famiglia',
            'available': 'Disponibile',
            'family_expenses': 'Spese Famiglia',
            'spent': 'Speso',
            'shopping_list': 'Lista della Spesa',
            'placeholder_add_product': 'Aggiungi prodotto...',
            'recent_movements': 'Movimenti Recenti',
            'no_recent_movements': 'Nessun movimento recente.',
            'day_events': 'Eventi del Giorno',
            'delete_all': 'Elimina Tutti',
            'add_event': 'Aggiungi Evento',
            'select_category': 'Seleziona Categoria',
            'amount_currency': 'Importo (€)',
            'save_event': 'Salva Evento',
            'smart_import_desc': 'Incolla qui il testo del tuo estratto conto oppure carica un file (CSV o TXT). Analizzerò automaticamente importi e descrizioni per suddividerli nelle categorie corrette.',
            'drag_drop_file': 'Trascina qui o clicca per caricare un file',
            'support_formats': 'Supporta .csv e .txt',
            'placeholder_smart_import': 'Oppure incolla il testo qui...\nEsempio:\n10/03/2026 AMAZON MARKETPLACE -45.90',
            'analysis_results': 'Risultati Analisi',
            'analyze_text': 'Analizza Testo',
            'confirm_apply': 'Conferma e Applica',
            'goal_deposit_title': 'Versamento Obiettivo',
            'amount_to_deposit': 'Importo da depositare',
            'payment_method': 'Metodo di pagamento',
            'cash': 'Contanti',
            'card': 'Carta',
            'bank_transfer': 'Bonifico',
            'confirm_deposit': 'Conferma Deposito',
            'profile_pic': 'Foto Profilo',
            'choose_gallery': 'Scegli dalla Galleria',
            'take_photo': 'Scatta una Foto',
            // Family Page Additions
            'live_sync': 'Sincronizzazione Live',
            'live_sync_desc': 'Ogni spesa aggiunta è visibile istantaneamente.',
            'expense_attribution': 'Attribuzione Spese',
            'expense_attribution_desc': 'Saprete sempre chi ha inserito ogni movimento.',
            'or': 'OPPURE',
            'placeholder_enter_code': 'Inserisci Codice...',
            'join': 'Unisci',
            'family_finance': 'Finanza Familiare',
            'family_finance_desc': 'Collaborazione in tempo reale per le spese di casa.',
            'shared_dashboard': 'Dashboard Condiviso:',
            'copy': 'COPIA',
            'leave': 'ESCI',
            'family_summary': 'Resoconto Famigliare',
            'shared_remainder': 'Rimanenza Comune',
            'family_finance_caps': 'FINANZA FAMIGLIA',
            'family_members': 'Membri della Famiglia',
            'family_collaboration_desc': 'Collaborazione in tempo reale. I membri possono vedere e modificare il budget.',
            'common_monthly_income': 'Reddito Comune Mensile (€)',
            'placeholder_income_f': 'Es. 3000'
        },
        'en': {
            'app_title': 'FinanceAI - Your Financial Assistant',
            'app_name': 'Easy Finance',
            'choose_account': 'Choose an Account',
            'add_account': 'Add Account',
            'use_another_profile': 'Use another profile',
            'login': 'Login',
            'placeholder_email': 'Enter your email',
            'placeholder_password': 'Enter your password',
            'placeholder_new_password': 'Choose a password',
            'placeholder_username': 'Choose a username',
            'remember_me': 'Remember Account on this device',
            'login_button': 'Login now',
            'no_account': 'Don\'t have an account?',
            'signup_link': 'Sign up',
            'cancel': 'Cancel',
            'signup': 'Sign up',
            'min_chars': 'Minimum 6 characters',
            'create_account': 'Create Account',
            'have_account': 'Already have an account?',
            'login_link': 'Login',
            'current_accounts': 'Current Accounts',
            'settings': 'Settings',
            'logout': 'Logout',
            'download_app': 'Download App',
            'appearance': 'Appearance',
            'nav_home': 'Home',
            'nav_budget': 'Budget',
            'nav_goals': 'Goals',
            'nav_family': 'Family',
            'nav_stats': 'Stats',
            'nav_settings': 'Settings',
            'welcome_title': 'Welcome to your Financial Center',
            'welcome_desc': 'Easy Finance is your all-in-one platform for complete financial control.',
            'spending_breakdown': 'Current Spending Breakdown',
            'monthly_status': 'Monthly Status',
            'guide_budget_title': 'Budget & AI',
            'guide_budget_desc': 'Plan income and expenses with the help of our intelligent algorithm to maximize your savings every month.',
            'guide_goals_title': 'Goals & Calendar',
            'guide_goals_desc': 'Sync your savings goals with the dynamic calendar and track every deposit in real-time.',
            'guide_stats_title': 'Analysis & Future',
            'guide_stats_desc': 'Graphically visualize your savings growth and discover how to reach your financial goals faster.',
            'ready_start': 'Ready to start?',
            'ready_desc': 'Head to the Budget & Calendar section to set up your finances for this month.',
            'go_to_budget': 'Go to Budget',
            'monthly_budget': 'Your Monthly Budget',
            'download_csv_demo': 'Download Demo CSV',
            'smart_import': 'Smart Import',
            'net_monthly_income': 'Net Monthly Income (€)',
            'placeholder_income': 'E.g. 2000',
            'transfer_to_family': 'Transfer to Family',
            'transfer_desc': 'Withdraw from your personal account and add to the shared budget.',
            'transfer': 'Transfer',
            'fixed_monthly_expenses': 'Fixed Monthly Expenses',
            'cat_rent': 'Rent / Mortgage',
            'cat_bills': 'Bills',
            'cat_auto': 'Auto / Transport',
            'cat_food': 'Groceries / Food',
            'cat_subs': 'Subscriptions',
            'cat_leisure': 'Leisure',
            'cat_shopping': 'Shopping',
            'cat_extra': 'Extra',
            'cat_income': 'Salary',
            'summary': 'Summary',
            'income': 'Income',
            'expenses': 'Expenses',
            'available_balance': 'Available Balance (Remaining)',
            'user': 'User',
            'saving_tips': 'Saving Tips',
            'mon': 'Mon', 'tue': 'Tue', 'wed': 'Wed', 'thu': 'Thu', 'fri': 'Fri', 'sat': 'Sat', 'sun': 'Sun',
            'savings_goals': 'Savings Goals',
            'new_goal': 'New Goal',
            'goal_name': 'Goal Name',
            'placeholder_goal_name': 'e.g. Vacation, New Car',
            'total_cost': 'Total Cost (€)',
            'placeholder_0': '0',
            'target_time': 'Target Time (months)',
            'monthly_deposit': 'Monthly Deposit (€)',
            'placeholder_100': 'e.g. 100',
            'add': 'Add',
            'yearly_stats': 'Yearly Statistics',
            'total_yearly_salary': 'Total Yearly Salary',
            'total_yearly_savings': 'Total Yearly Savings',
            'chart_click_desc': 'Click on a pie chart slice to view the expense category details here.',
            'monthly_savings_year': 'Monthly Savings of the Year',
            'account_settings': 'Account Settings',
            'user_profile': 'User Profile',
            'profile_desc': 'Edit your account information. These changes will be visible across all sections of the app.',
            'change_profile_pic': 'Change your profile picture',
            'email_addr': 'Email Address (Non-editable)',
            'placeholder_loading': 'Loading...',
            'username': 'Username',
            'save_changes': 'Save Changes',
            'card_style': 'Card Style',
            'card_style_desc': 'Customize the appearance of your virtual card in the dashboard.',
            'preview': 'Preview',
            'username_caps': 'USERNAME',
            'bg_customization': 'Background Customization',
            'bg_customization_desc': 'Choose an exclusive theme for your financial space.',
            'language': 'Language',
            'language_desc': 'Choose the application language.',
            'connected_to_family': 'You are connected to your Family!',
            'family_sync_desc': 'Sync budget, goals, and calendar with your family group members.',
            'loading': 'Loading...',
            'no_family_prompt': 'You are not part of a family group yet. Create a new group or ask a family member to invite you via your email.',
            'create_family_group': 'Create Family Group',
            'family_budget': 'Family Budget',
            'available': 'Available',
            'family_expenses': 'Family Expenses',
            'spent': 'Spent',
            'shopping_list': 'Shopping List',
            'placeholder_add_product': 'Add product...',
            'recent_movements': 'Recent Movements',
            'no_recent_movements': 'No recent movements.',
            'day_events': 'Day Events',
            'delete_all': 'Delete All',
            'add_event': 'Add Event',
            'select_category': 'Select Category',
            'amount_currency': 'Amount (€)',
            'save_event': 'Save Event',
            'smart_import_desc': 'Paste your bank statement text here or upload a file (CSV or TXT). I will automatically analyze amounts and descriptions to categorize them correctly.',
            'drag_drop_file': 'Drag here or click to upload a file',
            'support_formats': 'Supports .csv and .txt',
            'placeholder_smart_import': 'Or paste text here...\nExample:\n10/03/2026 AMAZON MARKETPLACE -45.90',
            'analysis_results': 'Analysis Results',
            'analyze_text': 'Analyze Text',
            'confirm_apply': 'Confirm and Apply',
            'goal_deposit_title': 'Goal Deposit',
            'amount_to_deposit': 'Amount to deposit',
            'payment_method': 'Payment method',
            'cash': 'Cash',
            'card': 'Card',
            'bank_transfer': 'Bank Transfer',
            'confirm_deposit': 'Confirm Deposit',
            'profile_pic': 'Profile Picture',
            'choose_gallery': 'Choose from Gallery',
            'take_photo': 'Take a Photo',
            // Family Page Additions
            'live_sync': 'Live Sync',
            'live_sync_desc': 'Every expense added is instantly visible.',
            'expense_attribution': 'Expense Attribution',
            'expense_attribution_desc': 'You will always know who entered each movement.',
            'or': 'OR',
            'placeholder_enter_code': 'Enter Code...',
            'join': 'Join',
            'family_finance': 'Family Finance',
            'family_finance_desc': 'Real-time collaboration for home expenses.',
            'shared_dashboard': 'Shared Dashboard:',
            'copy': 'COPY',
            'leave': 'LEAVE',
            'family_summary': 'Family Summary',
            'shared_remainder': 'Shared Remainder',
            'family_finance_caps': 'FAMILY FINANCE',
            'family_members': 'Family Members',
            'family_collaboration_desc': 'Real-time collaboration. Members can see and edit the budget.',
            'common_monthly_income': 'Common Monthly Income (€)',
            'placeholder_income_f': 'e.g. 3000'
        },
        'es': {
            'app_title': 'FinanceAI - Tu Asistente Financiero',
            'app_name': 'Finanza Fácil',
            'choose_account': 'Elige una Cuenta',
            'add_account': 'Añadir Cuenta',
            'use_another_profile': 'Usar otro perfil',
            'login': 'Acceder',
            'placeholder_email': 'Introduce tu email',
            'placeholder_password': 'Introduce tu contraseña',
            'placeholder_new_password': 'Elige una contraseña',
            'placeholder_username': 'Elige un nombre de usuario',
            'remember_me': 'Recordar Cuenta en este dispositivo',
            'login_button': 'Acceder ahora',
            'no_account': '¿No tienes una cuenta?',
            'signup_link': 'Regístrate',
            'cancel': 'Cancelar',
            'signup': 'Regístrate',
            'min_chars': 'Mínimo 6 caracteres',
            'create_account': 'Crear Cuenta',
            'have_account': '¿Ya tienes una cuenta?',
            'login_link': 'Acceder',
            'current_accounts': 'Cuentas Actuales',
            'settings': 'Ajustes',
            'logout': 'Salir',
            'download_app': 'Descargar App',
            'appearance': 'Apariencia',
            'nav_home': 'Inicio',
            'nav_budget': 'Presupuesto',
            'nav_goals': 'Objetivos',
            'nav_family': 'Familia',
            'nav_stats': 'Stats',
            'nav_settings': 'Ajustes',
            'welcome_title': 'Bienvenido a tu Centro Financiero',
            'welcome_desc': 'Finanza Fácil es tu plataforma todo en uno para el control financiero completo.',
            'spending_breakdown': 'Desglose de Gastos Actuales',
            'monthly_status': 'Estado Mensual',
            'guide_budget_title': 'Presupuesto & IA',
            'guide_budget_desc': 'Planifica ingresos y gastos con la ayuda de nuestro algoritmo inteligente para maximizar tus ahorros cada mes.',
            'guide_goals_title': 'Objetivos & Calendario',
            'guide_goals_desc': 'Sincroniza tus objetivos de ahorro con el calendario dinámico y rastrea cada depósito en tiempo real.',
            'guide_stats_title': 'Análisis & Futuro',
            'guide_stats_desc': 'Visualiza gráficamente el crecimiento de tus ahorros y descubre cómo alcanzar tus metas financieras más rápido.',
            'ready_start': '¿Listo para empezar?',
            'ready_desc': 'Dirígete a la sección Presupuesto & Calendario para configurar tus finanzas de este mes.',
            'go_to_budget': 'Ir al Presupuesto',
            'monthly_budget': 'Tu Presupuesto Mensual',
            'download_csv_demo': 'Descargar CSV Demo',
            'smart_import': 'Importación Inteligente',
            'net_monthly_income': 'Ingresos Mensuales Netos (€)',
            'placeholder_income': 'Ej. 2000',
            'transfer_to_family': 'Transferir a la Familia',
            'transfer_desc': 'Retira de tu cuenta personal y añade al presupuesto común.',
            'transfer': 'Transferir',
            'fixed_monthly_expenses': 'Gastos Mensuales Fijos',
            'cat_rent': 'Alquiler / Hipoteca',
            'cat_bills': 'Facturas',
            'cat_auto': 'Auto / Transporte',
            'cat_food': 'Súper / Comida',
            'cat_subs': 'Suscripciones',
            'cat_leisure': 'Ocio',
            'cat_shopping': 'Compras',
            'cat_extra': 'Extra',
            'cat_income': 'Salario',
            'summary': 'Resumen',
            'income': 'Ingresos',
            'expenses': 'Gastos',
            'available_balance': 'Saldo Disponible (Restante)',
            'user': 'Usuario',
            'saving_tips': 'Consejos para Ahorrar',
            'mon': 'Lun', 'tue': 'Mar', 'wed': 'Mié', 'thu': 'Jue', 'fri': 'Vie', 'sat': 'Sáb', 'sun': 'Dom',
            'savings_goals': 'Objetivos de Ahorro',
            'new_goal': 'Nuevo Objetivo',
            'goal_name': 'Nombre del Objetivo',
            'placeholder_goal_name': 'ej. Vacaciones, Coche Nuevo',
            'total_cost': 'Coste Total (€)',
            'placeholder_0': '0',
            'target_time': 'Tiempo Objetivo (meses)',
            'monthly_deposit': 'Depósito Mensual (€)',
            'placeholder_100': 'ej. 100',
            'add': 'Añadir',
            'yearly_stats': 'Estadísticas Anuales',
            'total_yearly_salary': 'Salario Total Anual',
            'total_yearly_savings': 'Ahorro Total Anual',
            'chart_click_desc': 'Haz clic en una porción del gráfico para ver los detalles de la categoría aquí.',
            'monthly_savings_year': 'Ahorros Mensuales del Año',
            'account_settings': 'Ajustes de Cuenta',
            'user_profile': 'Perfil de Usuario',
            'profile_desc': 'Edita la información de tu cuenta. Estos cambios serán visibles en todas las secciones.',
            'change_profile_pic': 'Cambia tu foto de perfil',
            'email_addr': 'Correo Electrónico (No editable)',
            'placeholder_loading': 'Cargando...',
            'username': 'Usuario',
            'save_changes': 'Guardar Cambios',
            'card_style': 'Estilo de Tarjeta',
            'card_style_desc': 'Personaliza la apariencia de tu tarjeta virtual en el dashboard.',
            'preview': 'Vista Previa',
            'username_caps': 'USUARIO',
            'bg_customization': 'Personalización de Fondo',
            'bg_customization_desc': 'Elige un tema exclusivo para tu espacio financiero.',
            'language': 'Idioma',
            'language_desc': 'Elige el idioma de la aplicación.',
            'connected_to_family': '¡Estás conectado a tu Familia!',
            'family_sync_desc': 'Sincroniza presupuesto, objetivos y calendario con los miembros de tu grupo familiar.',
            'loading': 'Cargando...',
            'no_family_prompt': 'Aún no formas parte de un grupo familiar. Crea un grupo o pide a un familiar que te invite.',
            'create_family_group': 'Crear Grupo Familiar',
            'family_budget': 'Presupuesto Familiar',
            'available': 'Disponible',
            'family_expenses': 'Gastos Familiares',
            'spent': 'Gastado',
            'shopping_list': 'Lista de la Compra',
            'placeholder_add_product': 'Añadir producto...',
            'recent_movements': 'Movimientos Recientes',
            'no_recent_movements': 'No hay movimientos recientes.',
            'day_events': 'Eventos del Día',
            'delete_all': 'Eliminar Todos',
            'add_event': 'Añadir Evento',
            'select_category': 'Seleccionar Categoría',
            'amount_currency': 'Importe (€)',
            'save_event': 'Guardar Evento',
            'smart_import_desc': 'Pega el texto de tu extracto bancario aquí o sube un archivo (CSV o TXT). Analizaré automáticamente importes y descripciones para categorizarlos correctamente.',
            'drag_drop_file': 'Arrastra aquí o haz clic para subir un archivo',
            'support_formats': 'Soporta .csv y .txt',
            'placeholder_smart_import': 'O pega el texto aquí...\nEjemplo:\n10/03/2026 AMAZON MARKETPLACE -45.90',
            'analysis_results': 'Resultados del Análisis',
            'analyze_text': 'Analizar Texto',
            'confirm_apply': 'Confirmar y Aplicar',
            'goal_deposit_title': 'Depósito de Objetivo',
            'amount_to_deposit': 'Importe a depositar',
            'payment_method': 'Método de pago',
            'cash': 'Efectivo',
            'card': 'Tarjeta',
            'bank_transfer': 'Transferencia',
            'confirm_deposit': 'Confirmar Depósito',
            'profile_pic': 'Foto de Perfil',
            'choose_gallery': 'Elegir de la Galería',
            'take_photo': 'Hacer una Foto',
            // Family Page Additions
            'live_sync': 'Sincronización en Vivo',
            'live_sync_desc': 'Cada gasto añadido es visible al instante.',
            'expense_attribution': 'Atribución de Gastos',
            'expense_attribution_desc': 'Siempre sabrás quién introdujo cada movimiento.',
            'or': 'O BIEN',
            'placeholder_enter_code': 'Introduce el Código...',
            'join': 'Unirse',
            'family_finance': 'Finanzas Familiares',
            'family_finance_desc': 'Colaboración en tiempo real para los gastos del hogar.',
            'shared_dashboard': 'Panel Compartido:',
            'copy': 'COPIAR',
            'leave': 'SALIR',
            'family_summary': 'Resumen Familiar',
            'shared_remainder': 'Remanente Común',
            'family_finance_caps': 'FINANZAS FAMILIARES',
            'family_members': 'Miembros de la Familia',
            'family_collaboration_desc': 'Colaboración en tiempo real. Los miembros pueden ver y editar el presupuesto.',
            'common_monthly_income': 'Ingresos Comunes Mensuales (€)',
            'placeholder_income_f': 'ej. 3000'
        },
        'fr': {
            'app_title': 'FinanceAI - Votre Assistant Financier',
            'app_name': 'Finance Facile',
            'choose_account': 'Choisissez un Compte',
            'add_account': 'Ajouter un Compte',
            'use_another_profile': 'Utiliser un autre profil',
            'login': 'Se Connecter',
            'placeholder_email': 'Entrez votre email',
            'placeholder_password': 'Entrez votre mot de passe',
            'placeholder_new_password': 'Choisissez un mot de passe',
            'placeholder_username': 'Choisissez un nom d\'utilisateur',
            'remember_me': 'Se souvenir du compte sur cet appareil',
            'login_button': 'Se connecter maintenant',
            'no_account': 'Pas de compte ?',
            'signup_link': 'S\'inscrire',
            'cancel': 'Annuler',
            'signup': 'S\'inscrire',
            'min_chars': 'Minimum 6 caractères',
            'create_account': 'Créer un Compte',
            'have_account': 'Déjà un compte ?',
            'login_link': 'Connexion',
            'current_accounts': 'Comptes Actuels',
            'settings': 'Paramètres',
            'logout': 'Déconnexion',
            'download_app': 'Télécharger l\'App',
            'appearance': 'Apparence',
            'nav_home': 'Accueil',
            'nav_budget': 'Budget',
            'nav_goals': 'Objectifs',
            'nav_family': 'Famille',
            'nav_stats': 'Stats',
            'nav_settings': 'Paramètres',
            'welcome_title': 'Bienvenue dans votre Centre Financier',
            'welcome_desc': 'Finance Facile est votre plateforme tout-en-un pour un contrôle financier complet.',
            'spending_breakdown': 'Répartition des Dépenses Actuelles',
            'monthly_status': 'État Mensuel',
            'guide_budget_title': 'Budget & IA',
            'guide_budget_desc': 'Planifiez vos revenus et dépenses avec l\'aide de notre algorithme intelligent pour maximiser vos économies chaque mois.',
            'guide_goals_title': 'Objectifs & Calendrier',
            'guide_goals_desc': 'Synchronisez vos objectifs d\'épargne avec le calendrier dynamique et suivez chaque dépôt en temps réel.',
            'guide_stats_title': 'Analyse & Futur',
            'guide_stats_desc': 'Visualisez graphiquement la croissance de votre épargne et découvrez comment atteindre vos objectifs financiers plus rapidement.',
            'ready_start': 'Prêt à commencer ?',
            'ready_desc': 'Rendez-vous dans la section Budget & Calendrier pour configurer vos finances ce mois-ci.',
            'go_to_budget': 'Aller au Budget',
            'monthly_budget': 'Votre Budget Mensuel',
            'download_csv_demo': 'Télécharger CSV Démo',
            'smart_import': 'Importation Intelligente',
            'net_monthly_income': 'Revenu Mensuel Net (€)',
            'placeholder_income': 'Ex. 2000',
            'transfer_to_family': 'Transférer à la Famille',
            'transfer_desc': 'Retirez de votre compte personnel et ajoutez au budget commun.',
            'transfer': 'Transférer',
            'fixed_monthly_expenses': 'Dépenses Mensuelles Fixes',
            'cat_rent': 'Loyer / Hypothèque',
            'cat_bills': 'Factures',
            'cat_auto': 'Auto / Transport',
            'cat_food': 'Courses / Nourriture',
            'cat_subs': 'Abonnements',
            'cat_leisure': 'Loisirs',
            'cat_shopping': 'Shopping',
            'cat_extra': 'Extra',
            'cat_income': 'Salaire',
            'summary': 'Résumé',
            'income': 'Revenus',
            'expenses': 'Dépenses',
            'available_balance': 'Solde Disponible (Reste)',
            'user': 'Utilisateur',
            'saving_tips': 'Conseils pour Économiser',
            'mon': 'Lun', 'tue': 'Mar', 'wed': 'Mer', 'thu': 'Jeu', 'fri': 'Ven', 'sat': 'Sam', 'sun': 'Dim',
            'savings_goals': 'Objectifs d\'Épargne',
            'new_goal': 'Nouvel Objectif',
            'goal_name': 'Nom de l\'Objectif',
            'placeholder_goal_name': 'ex. Vacances, Voiture Neuve',
            'total_cost': 'Coût Total (€)',
            'placeholder_0': '0',
            'target_time': 'Temps Cible (mois)',
            'monthly_deposit': 'Dépôt Mensuel (€)',
            'placeholder_100': 'ex. 100',
            'add': 'Ajouter',
            'yearly_stats': 'Statistiques Annuelles',
            'total_yearly_salary': 'Salaire Annuel Total',
            'total_yearly_savings': 'Épargne Annuelle Totale',
            'chart_click_desc': 'Cliquez sur une part du graphique pour voir les détails de la catégorie ici.',
            'monthly_savings_year': 'Épargne Mensuelle de l\'Année',
            'account_settings': 'Paramètres du Compte',
            'user_profile': 'Profil Utilisateur',
            'profile_desc': 'Modifiez vos informations de compte. Ces modifications seront visibles dans toutes les sections.',
            'change_profile_pic': 'Changer votre photo de profil',
            'email_addr': 'Adresse Email (Non modifiable)',
            'placeholder_loading': 'Chargement...',
            'username': 'Utilisateur',
            'save_changes': 'Enregistrer les Modifications',
            'card_style': 'Style de Carte',
            'card_style_desc': 'Personnalisez l\'apparence de votre carte virtuelle dans le tableau de bord.',
            'preview': 'Aperçu',
            'username_caps': 'UTILISATEUR',
            'bg_customization': 'Personnalisation de l\'Arrière-plan',
            'bg_customization_desc': 'Choisissez un thème exclusif pour votre espace financier.',
            'language': 'Langue',
            'language_desc': 'Choisissez la langue de l\'application.',
            'connected_to_family': 'Vous êtes connecté à votre Famille !',
            'family_sync_desc': 'Synchronisez le budget, les objectifs et le calendrier avec les membres de votre famille.',
            'loading': 'Chargement...',
            'no_family_prompt': 'Vous ne faites pas encore partie d\'un groupe. Créez un groupe ou demandez à un proche de vous inviter.',
            'create_family_group': 'Créer un Groupe Familial',
            'family_budget': 'Budget Famille',
            'available': 'Disponible',
            'family_expenses': 'Dépenses Famille',
            'spent': 'Dépensé',
            'shopping_list': 'Liste de Courses',
            'placeholder_add_product': 'Ajouter produit...',
            'recent_movements': 'Mouvements Récents',
            'no_recent_movements': 'Aucun mouvement récent.',
            'day_events': 'Événements du Jour',
            'delete_all': 'Supprimer Tout',
            'add_event': 'Ajouter Événement',
            'select_category': 'Sélectionner Catégorie',
            'amount_currency': 'Montant (€)',
            'save_event': 'Enregistrer Événement',
            'smart_import_desc': 'Collez le texte de votre relevé bancaire ici ou téléchargez un fichier (CSV ou TXT). J\'analyserai automatiquement les montants pour les catégoriser.',
            'drag_drop_file': 'Glissez ici ou cliquez pour télécharger',
            'support_formats': 'Supporte .csv et .txt',
            'placeholder_smart_import': 'Ou collez le texte ici...\nExemple :\n10/03/2026 AMAZON MARKETPLACE -45.90',
            'analysis_results': 'Résultats d\'Analyse',
            'analyze_text': 'Analyser Texte',
            'confirm_apply': 'Confirmer et Appliquer',
            'goal_deposit_title': 'Dépôt pour Objectif',
            'amount_to_deposit': 'Montant à déposer',
            'payment_method': 'Méthode de paiement',
            'cash': 'Espèces',
            'card': 'Carte',
            'bank_transfer': 'Virement',
            'confirm_deposit': 'Confirmer Dépôt',
            'profile_pic': 'Photo de Profil',
            'choose_gallery': 'Choisir dans la Galerie',
            'take_photo': 'Prendre une Photo',
            // Family Page Additions
            'live_sync': 'Synchronisation en Direct',
            'live_sync_desc': 'Chaque dépense ajoutée est instantanément visible.',
            'expense_attribution': 'Attribution des Dépenses',
            'expense_attribution_desc': 'Vous saurez toujours qui a saisi chaque mouvement.',
            'or': 'OU BIEN',
            'placeholder_enter_code': 'Entrez le Code...',
            'join': 'Rejoindre',
            'family_finance': 'Finance Familiale',
            'family_finance_desc': 'Collaboration en temps réel pour le budget domestique.',
            'shared_dashboard': 'Tableau de Bord Partagé :',
            'copy': 'COPIER',
            'leave': 'QUITTER',
            'family_summary': 'Résumé Familial',
            'shared_remainder': 'Reste Commun',
            'family_finance_caps': 'FINANCE FAMILIALE',
            'family_members': 'Membres de la Famille',
            'family_collaboration_desc': 'Collaboration en temps réel. Les membres peuvent voir et modifier le budget.',
            'common_monthly_income': 'Revenu Mensuel Commun (€)',
            'placeholder_income_f': 'ex. 3000'
        }
    };

    function applyTranslations(lang = 'it') {
        document.querySelectorAll('[data-i18n]').forEach(el => {
            const key = el.getAttribute('data-i18n');
            if (translations[lang] && translations[lang][key]) {
                // Preserva l'eventuale icona i all'inizio
                const icon = el.querySelector('i');
                if (icon) {
                    // Se l'elemento ha un'icona e un testo separato (es. span)
                    const span = el.querySelector('span');
                    if (span && span.getAttribute('data-i18n') === key) {
                        span.textContent = translations[lang][key];
                    } else {
                        // Se l'icona è figlia diretta, la teniamo e cambiamo il testo
                        // Cerchiamo il nodo di testo significativo (non solo whitespace)
                        const textNodes = Array.from(el.childNodes).filter(node => node.nodeType === Node.TEXT_NODE);
                        const mainTextNode = textNodes.find(node => node.textContent.trim().length > 0) || textNodes[textNodes.length - 1];
                        
                        if (mainTextNode) {
                            // Puliamo gli altri nodi di testo se ne abbiamo trovati di significativi
                            textNodes.forEach(node => {
                                if (node === mainTextNode) {
                                    node.textContent = translations[lang][key];
                                } else if (node.textContent.trim().length > 0) {
                                    // Se c'erano altri nodi con testo, li svuotiamo per evitare duplicati
                                    node.textContent = '';
                                }
                            });
                        }
                    }
                } else {
                    el.textContent = translations[lang][key];
                }
            }
        });

        document.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
            const key = el.getAttribute('data-i18n-placeholder');
            if (translations[lang] && translations[lang][key]) {
                el.placeholder = translations[lang][key];
            }
        });

        // Update document title
        if (translations[lang]['app_title']) {
            document.title = translations[lang]['app_title'];
        }

        // Update locale for currency and dates
        const localeMap = { 'it': 'it-IT', 'en': 'en-US', 'es': 'es-ES', 'fr': 'fr-FR' };
        const locale = localeMap[lang] || 'it-IT';
        
        window.currentCurrency = new Intl.NumberFormat(locale, {
            style: 'currency',
            currency: 'EUR'
        });
        
        // Update chart labels if existing
        if (dashboardPieChart) {
            const catKeys = ['rent', 'bills', 'auto', 'food', 'subs', 'leisure', 'shopping', 'extra'];
            dashboardPieChart.data.labels = catKeys.map(k => categoryNames[lang][k]);
            dashboardPieChart.update();
        }

        // Refresh dynamic UI components
        recalculateSummary();
        renderCalendar();
        if (typeof renderYearlyChart === 'function') renderYearlyChart();
        if (typeof initFamilyUI === 'function') initFamilyUI();
    }

    function changeLanguage(lang) {
        localStorage.setItem('financeLanguage', lang);
        document.documentElement.lang = lang;
        
        // Update active class on buttons
        document.querySelectorAll('.lang-btn').forEach(btn => {
            btn.classList.toggle('active', btn.getAttribute('data-lang') === lang);
            btn.style.border = btn.getAttribute('data-lang') === lang ? '2px solid var(--primary)' : '2px solid transparent';
        });

        applyTranslations(lang);
    }

    const categoryToType = {
        'income': 'income',
        'rent': 'expense',
        'bills': 'expense',
        'auto': 'expense',
        'food': 'expense',
        'subs': 'expense',
        'leisure': 'expense',
        'shopping': 'expense',
        'extra': 'expense',
        'other': 'other'
    };

    const currency = new Intl.NumberFormat('it-IT', {
        style: 'currency',
        currency: 'EUR'
    });

    let currentLanguage = localStorage.getItem('financeLanguage') || 'it';

    const getI18n = (key) => {
        return (translations[currentLanguage] && translations[currentLanguage][key]) || key;
    };

    const getCategoryName = (cat) => {
        return (categoryNames[currentLanguage] && categoryNames[currentLanguage][cat]) || cat;
    };

    const getStorageKey = (key) => {
        if (!currentUser) return null;
        return `${currentUser.email}_${key}`;
    };

    const allBudgetInputs = [incomeInput, rentInput, billsInput, autoInput, foodInput, subsInput, leisureInput, shoppingInput, extraInput];

    // --- Magic Sync State ---
    let lastInputValues = {};
    function initializeLastValues() {
        allBudgetInputs.forEach(input => {
            if (input) {
                lastInputValues[input.id] = parseFloat(input.value) || 0;
            }
        });
    }

    if (smartImportBtn) {
        smartImportBtn.addEventListener('click', () => {
            smartImportModal.classList.remove('hidden');
            smartImportTextArea.value = '';
            smartImportReview.classList.add('hidden');
            applySmartImportBtn.classList.add('hidden');
            smartFileUpload.value = '';
            smartFileName.classList.add('hidden');
            fileContent = "";
        });
    }

    if (closeSmartImportBtn) {
        closeSmartImportBtn.addEventListener('click', () => {
            smartImportModal.classList.add('hidden');
        });
    }

    if (smartFileUpload) {
        smartFileUpload.addEventListener('change', (e) => {
            const file = e.target.files[0];
            if (!file) return;

            fileNameSpan.textContent = file.name;
            smartFileName.classList.remove('hidden');

            const reader = new FileReader();
            reader.onload = (event) => {
                fileContent = event.target.result;
            };
            reader.readAsText(file);
        });
    }

    if (analyzeSmartImportBtn) {
        analyzeSmartImportBtn.addEventListener('click', () => {
            const pastedText = smartImportTextArea.value.trim();
            const textToAnalyze = fileContent || pastedText;

            if (!textToAnalyze) {
                alert("Per favore, incolla del testo o carica un file da analizzare.");
                return;
            }

            const results = processSmartImportText(textToAnalyze);
            if (results.count > 0) {
                smartImportData = results.categories;
                smartImportItemsList = results.items;
                renderSmartImportReview(results.items);
                smartImportReview.classList.remove('hidden');
                applySmartImportBtn.classList.remove('hidden');
            } else {
                alert("Non ho trovato nessun importo o descrizione valida nel contenuto fornito.");
            }
        });
    }

    if (applySmartImportBtn) {
        applySmartImportBtn.addEventListener('click', async () => {
            if (smartImportData.income > 0) incomeInput.value = ((parseFloat(incomeInput.value) || 0) + smartImportData.income).toFixed(2);
            if (smartImportData.rent > 0) rentInput.value = ((parseFloat(rentInput.value) || 0) + smartImportData.rent).toFixed(2);
            if (smartImportData.bills > 0) billsInput.value = ((parseFloat(billsInput.value) || 0) + smartImportData.bills).toFixed(2);
            if (smartImportData.auto > 0) autoInput.value = ((parseFloat(autoInput.value) || 0) + smartImportData.auto).toFixed(2);
            if (smartImportData.food > 0) foodInput.value = ((parseFloat(foodInput.value) || 0) + smartImportData.food).toFixed(2);
            if (smartImportData.subs > 0) subsInput.value = ((parseFloat(subsInput.value) || 0) + smartImportData.subs).toFixed(2);
            if (smartImportData.leisure > 0) leisureInput.value = ((parseFloat(leisureInput.value) || 0) + smartImportData.leisure).toFixed(2);
            if (smartImportData.shopping > 0) shoppingInput.value = ((parseFloat(shoppingInput.value) || 0) + smartImportData.shopping).toFixed(2);
            if (smartImportData.extra > 0) extraInput.value = ((parseFloat(extraInput.value) || 0) + smartImportData.extra).toFixed(2);

            // Add events to calendar
            smartImportItemsList.forEach(item => {
                let dateStr = item.date;
                if (!dateStr) {
                    const today = new Date();
                    dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
                }
                
                if (!events[dateStr]) events[dateStr] = [];
                events[dateStr].push({
                    category: item.category,
                    amount: Math.abs(item.amount),
                    desc: item.desc
                });
            });

            localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
            await syncData('events', events);
            renderCalendar();

            await saveCurrentMonthBudget();
            recalculateSummary();
            initializeLastValues();

            smartImportModal.classList.add('hidden');
            alert("Dati applicati con successo al tuo budget!");
        });
    }

    function processSmartImportText(text) {
        const lines = text.split('\n').map(l => l.trim()).filter(l => l.length > 0);
        let categories = {
            income: 0, rent: 0, bills: 0, auto: 0,
            food: 0, subs: 0, leisure: 0, shopping: 0, extra: 0
        };
        let items = [];
        let count = 0;

        lines.forEach(line => {
            const amountRegex = /([-+]?\s?\d{1,3}(?:\.\d{3})*(?:,\d{2})|[-+]?\s?\d+(?:\.\d{2})?)/g;
            const matches = line.match(amountRegex);

            if (matches) {
                let valStr = matches[matches.length - 1].replace(/\s/g, '');
                if (valStr.includes(',') && valStr.includes('.')) {
                    valStr = valStr.replace(/\./g, '').replace(',', '.');
                } else if (valStr.includes(',')) {
                    valStr = valStr.replace(',', '.');
                }
                const amount = parseFloat(valStr);

                if (!isNaN(amount) && amount !== 0) {
                    let desc = line.replace(matches[matches.length - 1], '').trim();
                    let itemDate = null;
                    const dateMatch = line.match(/(\d{1,2})[\/\-\.](\d{1,2})(?:[\/\-\.](\d{2,4}))?/);
                    if (dateMatch) {
                        let [fullDate, day, month, year] = dateMatch;
                        const today = new Date();
                        if (!year) year = today.getFullYear().toString();
                        if (year.length === 2) year = '20' + year;
                        day = day.padStart(2, '0');
                        month = month.padStart(2, '0');
                        itemDate = `${year}-${month}-${day}`;
                        desc = desc.replace(fullDate, '').trim();
                    }
                    const category = categorizeExpense(desc.toLowerCase(), amount);
                    if (category) {
                        categories[category] += Math.abs(amount);
                        items.push({ desc: desc || "Movimento senza descrizione", amount, category, date: itemDate });
                        count++;
                    }
                }
            }
        });
        return { categories, items, count };
    }

    function renderSmartImportReview(items) {
        smartImportResultsList.innerHTML = '';
        items.forEach(item => {
            const div = document.createElement('div');
            div.style.display = 'flex';
            div.style.justifyContent = 'space-between';
            div.style.padding = '8px 0';
            div.style.borderBottom = '1px solid rgba(255,255,255,0.05)';
            div.style.fontSize = '0.9rem';
            const isPositive = item.amount > 0;
            const color = isPositive ? 'var(--success)' : 'white';
            const catName = getCategoryName(item.category) || 'Extra';
            div.innerHTML = `
                <div style="flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; padding-right: 10px;">
                    <span style="color: var(--text-muted); font-size: 0.75rem; display: block; text-transform: uppercase;">${item.date ? `[${item.date.split('-').reverse().join('/')}] ` : ''}${catName}</span>
                    ${item.desc}
                </div>
                <div style="font-weight: 600; color: ${color};">
                    ${isPositive ? '+' : ''}${(window.currentCurrency || currency).format(item.amount)}
                </div>
            `;
            smartImportResultsList.appendChild(div);
        });
    }

    function categorizeExpense(desc, amount) {
        if (amount > 0 && (desc.includes('stipendio') || desc.includes('bonifico a tuo favore') || desc.includes('accredito') || desc.includes('rimborso'))) return 'income';
        const categories = {
            'rent': ['affitto', 'mutuo', 'immobiliare', 'condominio', 'canone'],
            'bills': ['enel', 'luce', 'gas', 'acqua', 'tim', 'vodafone', 'wind', 'iliad', 'bolletta', 'tari', 'ama', 'telecom', 'fastweb', 'acea'],
            'auto': ['benzina', 'agip', 'eni', 'q8', 'tamoil', 'telepass', 'assicurazione auto', 'meccanico', 'parcheggio', 'autostrada', 'trenitalia', 'atm', 'atac', 'carburante', 'diesel', 'ip'],
            'food': ['supermercato', 'coop', 'conad', 'esselunga', 'pam', 'carrefour', 'lidl', 'eurospin', 'md', 'spesa', 'macellaio', 'panificio', 'ristorante', 'pizzeria', 'mcdonald', 'burger king', 'bar', 'glovo', 'deliveroo', 'just eat', 'tigros', 'iper', 'bennet'],
            'subs': ['netflix', 'spotify', 'amazon prime', 'disney+', 'dazn', 'palestra', 'abbonamento', 'icloud', 'g-suite', 'mensile'],
            'shopping': ['amazon', 'zara', 'h&m', 'ikea', 'zalando', 'abbigliamento', 'scarpe', 'negozio', 'centro commerciale', 'tezenis', 'calzedonia', 'mediaworld', 'unieuro', 'euronics'],
            'leisure': ['cinema', 'teatro', 'concerto', 'ticketone', 'museo', 'viaggio', 'ryanair', 'booking', 'easyjet', 'tripadvisor', 'hotel']
        };
        for (const [cat, keywords] of Object.entries(categories)) {
            if (keywords.some(kw => desc.includes(kw))) return cat;
        }
        return amount > 0 ? 'income' : 'extra';
    }

    // --- Virtual Card Selection logic ---
    if (allVirtualCards) {
        const savedCardClass = localStorage.getItem(getStorageKey('financeCardStyleClass')) || 'card-style-default';
        allVirtualCards.forEach(card => {
            if (card.id !== 'virtual-card-family') {
                card.classList.forEach(cls => { if(cls.startsWith('card-style-')) card.classList.remove(cls); });
                card.classList.add(savedCardClass);
            } else {
                card.classList.forEach(cls => { if(cls.startsWith('card-style-')) card.classList.remove(cls); });
                card.classList.add('card-style-ocean');
            }
        });
    }

    if (styleOptions) {
        styleOptions.forEach(option => {
            option.addEventListener('click', async () => {
                styleOptions.forEach(opt => opt.classList.remove('active'));
                option.classList.add('active');
                const styleClass = Array.from(option.classList).find(c => c.startsWith('card-style-'));
                if (styleClass) {
                    allVirtualCards.forEach(card => {
                        if (card.id !== 'virtual-card-family') {
                            card.classList.forEach(cls => { if(cls.startsWith('card-style-')) card.classList.remove(cls); });
                            card.classList.add(styleClass);
                        }
                    });
                    localStorage.setItem(getStorageKey('financeCardStyleClass'), styleClass);
                    if (supabase) await syncData('card_style_class', styleClass);
                }
            });
        });
    }

    const settingsUsernameInput = document.getElementById('settings-username');
    const previewCardUsername = document.getElementById('preview-card-username');
    if (settingsUsernameInput && previewCardUsername) {
        settingsUsernameInput.addEventListener('input', () => {
            previewCardUsername.textContent = settingsUsernameInput.value.toUpperCase() || 'USERNAME';
        });
    }
    // Summary
    const summarySection = document.getElementById('summary-section');
    const summaryIncome = document.getElementById('summary-income');
    const summaryExpenses = document.getElementById('summary-expenses');
    const summaryRemaining = document.getElementById('summary-remaining');
    const adviceList = document.getElementById('advice-list');





    function recalculateSummary() {
        const income = parseFloat(incomeInput.value) || 0;

        // Calcolo totale spese
        const rent = parseFloat(rentInput.value) || 0;
        const bills = parseFloat(billsInput.value) || 0;
        const auto = parseFloat(autoInput.value) || 0;
        const food = parseFloat(foodInput.value) || 0;
        const subs = parseFloat(subsInput.value) || 0;
        const leisure = parseFloat(leisureInput.value) || 0;
        const shopping = parseFloat(shoppingInput.value) || 0;
        const extra = parseFloat(extraInput.value) || 0;

        const totalExpenses = rent + bills + auto + food + subs + leisure + shopping + extra;
        const remaining = income - totalExpenses;

        // Mostra risultati
        const fmtCurrency = window.currentCurrency || currency;
        summaryIncome.textContent = fmtCurrency.format(income);
        summaryExpenses.textContent = fmtCurrency.format(totalExpenses);
        summaryRemaining.textContent = fmtCurrency.format(remaining);

        if (remaining < 0) {
            summaryRemaining.style.color = 'var(--danger)';
        } else {
            summaryRemaining.style.color = 'var(--text-main)';
        }

        // Update Dashboard Analytics
        updateDashboardChart();
        updateDashboardStatus(income, totalExpenses, remaining);

        // Genera suggerimenti
        generateAdvice(income, rent, bills, auto, food, totalExpenses, remaining);

        // Mostra la sezione
        if (summarySection) summarySection.classList.remove('hidden');
    }

    async function updateAvatarUI(base64Image, shouldSave = false) {
        const headerContainer = document.querySelector('.user-profile-btn');
        const settingsPreviewContainer = document.querySelector('.avatar-preview-container');

        if (base64Image) {
            // 1. Update UI Elements
            // Update or create <img> for header
            let headerImg = headerContainer.querySelector('.user-avatar');
            if (!headerImg) {
                headerContainer.querySelector('i')?.remove();
                headerImg = document.createElement('img');
                headerImg.className = 'user-avatar';
                headerContainer.appendChild(headerImg);
            }
            headerImg.src = base64Image;

            // Update <img> for settings preview
            if (settingsAvatarPreview) {
                settingsAvatarPreview.src = base64Image;
                settingsAvatarPreview.classList.remove('hidden');
                settingsPreviewContainer.querySelector('.fa-circle-user')?.classList.add('hidden');
            }

            // 2. Immediate Persistence if requested
            if (shouldSave && currentUser) {
                console.log("Salvataggio immediato foto profilo...");
                localStorage.setItem(getStorageKey('financeProfilePic'), base64Image);
                // Update financeLoggedUsers to reflect current picture for switching accounts
                let loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
                const lIdx = loggedUsers.findIndex(u => u.email === currentUser.email);
                if (lIdx >= 0) {
                    localStorage.setItem(`${currentUser.email}_financeProfilePic`, base64Image);
                }

                if (supabase) {
                    await syncData('profile_picture', base64Image);
                }
            }
        } else {
            // Fallback to Icon
            if (headerContainer) {
                headerContainer.querySelector('.user-avatar')?.remove();
                if (!headerContainer.querySelector('i')) {
                    const icon = document.createElement('i');
                    icon.className = 'fa-solid fa-circle-user';
                    headerContainer.appendChild(icon);
                }
            }
            if (settingsAvatarPreview) {
                settingsAvatarPreview.classList.add('hidden');
                let settingsIcon = settingsPreviewContainer.querySelector('.fa-circle-user');
                if (!settingsIcon) {
                    settingsIcon = document.createElement('i');
                    settingsIcon.className = 'fa-solid fa-circle-user';
                    settingsIcon.style.fontSize = '120px';
                    settingsIcon.style.color = 'var(--text-muted)';
                    settingsPreviewContainer.prepend(settingsIcon);
                }
                settingsIcon.classList.remove('hidden');
            }
        }
        
        // Also update the account list in the dropdown
        updateLoggedUsersList();
    }

    function updateDashboardChart() {
        const ctx = document.getElementById('dashboard-pie-chart');
        if (!ctx || !window.Chart) return;

        const dataValues = [
            parseFloat(rentInput.value) || 0,
            parseFloat(billsInput.value) || 0,
            parseFloat(autoInput.value) || 0,
            parseFloat(foodInput.value) || 0,
            parseFloat(subsInput.value) || 0,
            parseFloat(leisureInput.value) || 0,
            parseFloat(shoppingInput.value) || 0,
            parseFloat(extraInput.value) || 0
        ];

        const labels = ['Casa', 'Bollette', 'Trasporti', 'Cibo', 'Abbonamenti', 'Svago', 'Shopping', 'Extra'];
        const colors = ['#4a90e2', '#f39c12', '#e74c3c', '#2ecc71', '#9b59b6', '#1abc9c', '#f1c40f', '#34495e'];

        if (dashboardPieChart) {
            dashboardPieChart.data.datasets[0].data = dataValues;
            dashboardPieChart.update();
        } else {
            dashboardPieChart = new Chart(ctx, {
                type: 'doughnut',
                data: {
                    labels: labels,
                    datasets: [{
                        data: dataValues,
                        backgroundColor: colors,
                        borderColor: 'transparent',
                        borderWidth: 0,
                        hoverOffset: 15
                    }]
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: { display: false },
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    return `${context.label}: ${window.currentCurrency ? window.currentCurrency.format(context.raw) : currency.format(context.raw)}`;
                                }
                            }
                        }
                    },
                    cutout: '75%'
                }
            });
        }
    }

    function updateDashboardStatus(income, expenses, remaining) {
        const statusText = document.getElementById('dashboard-status-text');
        if (!statusText) return;

        if (income === 0) {
            statusText.innerHTML = `${getI18n('ready_desc')}`;
            return;
        }

        const percentUsed = ((expenses / income) * 100).toFixed(0);
        let message = `<div style="margin-bottom: 1rem;">Hai utilizzato il <b style="color: ${percentUsed > 90 ? 'var(--danger)' : 'var(--primary)'}">${percentUsed}%</b> del tuo budget.</div>`;
        
        if (remaining > 0) {
            message += `<div class="glass-panel" style="padding: 1rem; border-color: var(--success); background: rgba(16, 185, 129, 0.05);">
                Risparmio attuale previsto: <b style="color: var(--success); font-size: 1.2rem;">${currency.format(remaining)}</b>
            </div>`;
        } else if (remaining < 0) {
            message += `<div class="glass-panel" style="padding: 1rem; border-color: var(--danger); background: rgba(231, 76, 60, 0.05);">
                Sei in rosso di <b style="color: var(--danger); font-size: 1.2rem;">${currency.format(Math.abs(remaining))}</b>.<br>Riduci le spese opzionali immediatamente.
            </div>`;
        } else {
            message += `<div class="glass-panel" style="padding: 1rem;">Budget perfettamente bilanciato.</div>`;
        }

        statusText.innerHTML = message;
    }



    function generateAdvice(income, rent, bills, auto, food, totalExpenses, remaining) {
        adviceList.innerHTML = ''; // Svuota lista

        const advices = [];

        if (income === 0) {
            advices.push("Inserisci il tuo reddito per ricevere consigli personalizzati.");
            renderAdvices(advices);
            return;
        }

        if (remaining < 0) {
            advices.push("<strong>Attenzione:</strong> Le tue spese superano le entrate. Rivedi immediatamente le spese non essenziali.");
        }

        // Regola 50/30/20 approssimata per fisse
        const fixedPercentage = (totalExpenses / income) * 100;
        if (fixedPercentage > 60) {
            advices.push(`Le tue spese fisse assorbono il <strong>${fixedPercentage.toFixed(1)}%</strong> del tuo reddito (si consiglia max 50%). Prova a rinegoziare bollette o affitto.`);
        }

        // Controllo Affitto
        if ((rent / income) > 0.35) {
            advices.push("L'affitto/mutuo supera il 35% del tuo stipendio. Potrebbe incidere sui tuoi risparmi a lungo termine.");
        }

        // Controllo Auto
        if ((auto / income) > 0.15) {
            advices.push("Spendi molto in trasporti. Valuta il car sharing, abbonamenti pubblici o usa l'auto solo quando strettamente necessario.");
        }

        // Suggerimento Risparmio se in positivo
        if (remaining > 0) {
            let suggestSave = remaining * 0.5; // Suggerisci di salvare metà della rimanenza
            advices.push(`Ti rimangono ${currency.format(remaining)}. Prova a mettere da parte <strong>${currency.format(suggestSave)}</strong> questo mese in un fondo di emergenza.`);
        }

        if (advices.length === 0) {
            advices.push(getI18n('ready_desc'));
        }

        renderAdvices(advices);
    }

    function renderAdvices(advices) {
        advices.forEach(advice => {
            const li = document.createElement('li');
            li.innerHTML = advice;
            adviceList.appendChild(li);
        });
    }

    // --- Calendar Logic ---
    const calendarMonthYear = document.getElementById('calendar-month-year');
    const calendarGrid = document.getElementById('calendar-days-grid');
    const prevMonthBtn = document.getElementById('prev-month');
    const nextMonthBtn = document.getElementById('next-month');

    // --- Event Modals Elements ---
    const dayDetailsModal = document.getElementById('day-details-modal');
    const closeDetailsModalBtn = document.getElementById('close-details-modal');
    const dayDetailsTitle = document.getElementById('day-details-title');
    const dayEventsList = document.getElementById('day-events-list');
    const openAddEventBtn = document.getElementById('open-add-event-btn');
    const deleteAllEventsBtn = document.getElementById('delete-all-events-btn');

    const eventModal = document.getElementById('event-modal');
    const closeModalBtn = document.getElementById('close-modal');
    const saveEventBtn = document.getElementById('save-event-btn');
    const eventCategoryInput = document.getElementById('event-category');
    const eventAmountInput = document.getElementById('event-amount');

    let currentDate = new Date();
    let selectedDateInfo = null;

    // Carica budget mensili o array vuoto


    let monthlyBudgets = {};
    let events = {};
    let familyMonthlyBudgets = {};
    let familyEvents = {};
    async function loadUserData() {
        if (!currentUser) return;
        console.log("Sync: Performing loadUserData for", currentUser.email);

        try {
            const email = currentUser.email;
            let familyId = localStorage.getItem(`${email}_familyId`);

            // 1. Fetch Personal Data
            if (supabase && currentUser.id) {
                console.log("Sync: Personal data for", currentUser.username);
                const { data: pData, error: pError } = await supabase.from('user_data').select('key, value').eq('user_id', currentUser.id);
                if (pError) console.error("Sync Error (Personal):", pError);
                if (pData) {
                    pData.forEach(item => {
                        let pVal = item.value;
                        if (typeof pVal === 'string') {
                            try { pVal = JSON.parse(pVal); } catch(e){}
                        }

                        if (item.key === 'budgets') monthlyBudgets = pVal;
                        if (item.key === 'events') events = pVal;
                        if (item.key === 'profile_picture') {
                            localStorage.setItem(`${email}_financeProfilePic`, item.value);
                            updateAvatarUI(item.value);
                        }
                        if (item.key === 'background') {
                            localStorage.setItem(`${email}_financeBackground`, item.value);
                            if (typeof setBackground === 'function') setBackground(item.value);
                        }
                        if (item.key === 'theme') {
                            localStorage.setItem('financeTheme', item.value);
                            if (typeof setTheme === 'function') setTheme(item.value);
                        }
                        if (item.key === 'family_id') {
                            if (item.value && item.value !== 'null') {
                                // Only restore if we didn't JUST get expelled in this session
                                if (!sessionStorage.getItem('prevent_family_restore')) {
                                    console.log("Sync: Restoring family_id from cloud:", item.value);
                                    localStorage.setItem(`${email}_familyId`, item.value);
                                    familyId = item.value;
                                } else {
                                    console.log("Sync: Restoration blocked by session lock (just expelled)");
                                }
                            } else {
                                console.log("Sync: family_id is null/absent on cloud, clearing local.");
                                localStorage.removeItem(`${email}_familyId`);
                                familyId = null;
                            }
                        }
                    });
                }
            }

            // 2. Fetch Family Data (if code exists)
            if (supabase && familyId) {
                console.log("Sync: Family data for code", familyId);
                const { data: fData, error: fError } = await supabase.from('user_data').select('key, value').eq('user_id', familyId);
                if (fError) console.error("Sync Error (Family):", fError);
                if (fData) {
                    fData.forEach(item => {
                        let val = item.value;
                        if (typeof val === 'string') {
                            try { val = JSON.parse(val); } catch(e){}
                        }

                        if (item.key === 'family_budgets') familyMonthlyBudgets = val;
                        if (item.key === 'family_events') familyEvents = val;
                        if (item.key === 'members' || item.key === 'family_members') {
                            let mVal = item.value;
                            if (typeof mVal === 'string') {
                                try { mVal = JSON.parse(mVal); } catch(e){}
                            }
                            localStorage.setItem(`${familyId}_members`, JSON.stringify(mVal));
                        }
                    });
                }
            }

            // Update Local Storage
            localStorage.setItem(getStorageKey('financeMonthlyBudgets'), JSON.stringify(monthlyBudgets));
            localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
            if (familyId) {
                localStorage.setItem(`${familyId}_financeMonthlyBudgets`, JSON.stringify(familyMonthlyBudgets));
                localStorage.setItem(`${familyId}_financeEvents`, JSON.stringify(familyEvents));
            }

            // Refresh All UIs
            loadCurrentMonthBudget();
            renderCalendar();
            initFamilyUI();
            console.log("Sync: loadUserData completed successfully");
        } catch (err) {
            console.error("CRITICAL Sync Error in loadUserData:", err);
        }
    }

    async function syncData(key, value, forcedOwnerId = null) {
        if (!supabase || !currentUser) return;

        let ownerId = forcedOwnerId || currentUser.id;
        const familyId = localStorage.getItem(`${currentUser.email}_familyId`);
        
        // Rimossa logica di fallback automatico che causava sovrascrittura
        // dei dati familiari con i dati personali quando si salvava il budget personale
        // o un obiettivo mentre si era sulla tab famiglia.
        // I dati famiglia ora vengono salvati *esclusivamente* passando esplicitamente il familyId.

        const { error } = await supabase
            .from('user_data')
            .upsert({
                user_id: ownerId,
                key: key,
                value: value,
                updated_at: new Date().toISOString()
            }, { onConflict: 'user_id,key' });

        if (error) console.error(`Error syncing ${key} for ${ownerId}:`, error.message);
    }

    // --- Auth Logic ---
    function showAuthError(msg) {
        authError.textContent = msg;
        authError.classList.remove('hidden');
    }

    function toggleAuthForms(showSignup) {
        authError.classList.add('hidden');
        if (showSignup) {
            loginForm.classList.add('hidden');
            signupForm.classList.remove('hidden');
        } else {
            loginForm.classList.remove('hidden');
            signupForm.classList.add('hidden');
        }
    }

    async function handleSignup() {
        const emailInput = document.getElementById('signup-email');
        const emailValue = emailInput ? emailInput.value.trim() : '';
        const username = document.getElementById('signup-username').value.trim();
        const password = document.getElementById('signup-password').value.trim();

        if (!username || !password || (emailInput && !emailValue)) {
            showAuthError("Inserisci tutti i campi.");
            return;
        }

        if (supabase) {
            const { data, error } = await supabase.auth.signUp({
                email: emailValue,
                password: password,
                options: {
                    data: {
                        username: username,
                        real_email: emailValue
                    }
                }
            });

            if (error) {
                console.error("Signup Error Object:", error);
                
                let userFriendlyMsg = "Errore registrazione: " + error.message;
                if (error.message.includes("Password should be")) {
                    userFriendlyMsg = "La password deve essere di almeno 6 caratteri.";
                } else if (error.message.includes("User already registered")) {
                    userFriendlyMsg = "Questa email è già registrata. Prova ad accedere.";
                }
                
                showAuthError(userFriendlyMsg);
            } else {
                console.log("Signup Success:", data);
                alert("Registrazione completata! Se hai disattivato 'Confirm Email' nelle impostazioni di Supabase, ora puoi accedere.");
                toggleAuthForms(false);
            }
        } else {
            // Fallback locale se supabase non è pronto
            let users = JSON.parse(localStorage.getItem('financeUsers')) || [];
            if (users.find(u => u.username === username)) {
                showAuthError("Username già esistente.");
                return;
            }
            if (users.find(u => u.email === emailValue)) {
                showAuthError("Email già registrata localmente.");
                return;
            }
            users.push({ username, password, email: emailValue });
            localStorage.setItem('financeUsers', JSON.stringify(users));
            doLogin(username, null, emailValue);
        }
    }

    async function handleLogin() {
        const email = document.getElementById('login-email').value.trim();
        const password = document.getElementById('login-password').value.trim();

        if (!email || !password) {
            showAuthError("Inserisci tutti i campi.");
            return;
        }

        if (supabase) {
            const { data, error } = await supabase.auth.signInWithPassword({
                email: email,
                password: password
            });

            if (error) {
                showAuthError("Credenziali errate o errore: " + error.message);
            } else {
                // Prendi lo username dai metadata se possibile
                const user = data.user;
                const session = data.session;
                const finalUsername = user.user_metadata.username || email.split('@')[0];
                doLogin(finalUsername, user.id, email, session);
            }
        } else {
            let users = JSON.parse(localStorage.getItem('financeUsers')) || [];
            const user = users.find(u => u.email === email && u.password === password);
            if (user) {
                doLogin(user.username, null, email);
            } else {
                showAuthError("Email o password errati.");
            }
        }
    }

    async function doLogin(username, userId = null, email = null, session = null) {
        const rememberCheckbox = document.getElementById('login-remember');
        const shouldRemember = (rememberCheckbox) ? rememberCheckbox.checked : true;
        
        currentUser = { username, id: userId, email, session, remember: shouldRemember };
        localStorage.setItem('financeCurrentUser', JSON.stringify(currentUser));
        localStorage.setItem('financeLastUser', username);

        // Se vogliamo essere ricordati O eravamo già stati ricordati in precedenza,
        // aggiorniamo l'elenco degli account salvati.
        let loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
        const existingIdx = loggedUsers.findIndex(u => u.email === email);

        if (shouldRemember) {
            if (existingIdx >= 0) {
                loggedUsers[existingIdx] = currentUser;
            } else {
                loggedUsers.push(currentUser);
            }
            localStorage.setItem('financeLoggedUsers', JSON.stringify(loggedUsers));
        } else if (existingIdx >= 0) {
            loggedUsers[existingIdx] = currentUser;
            localStorage.setItem('financeLoggedUsers', JSON.stringify(loggedUsers));
        }

        await checkAuthStatus();
    }

    function switchAccount(user) {
        if (!user || !user.session) {
            console.warn("Auth: Impossibile cambiare account, sessione mancante.");
            alert("ERRORE CRITICO: Sessione mancante per " + (user ? user.email : "account sconosciuto"));
            return;
        }

        console.error("Auth: >>> AVVIO SWITCH ACCOUNT PER:", user.email, "<<<");
        
        // Pulizia totale e sicura di TUTTI i dati di sessione di Supabase nel localStorage
        Object.keys(localStorage).forEach(key => {
            if (key.startsWith('sb-') || key.includes('supabase-auth-token')) {
                localStorage.removeItem(key);
            }
        });

        // Impostiamo il nuovo utente come corrente
        localStorage.setItem('financeCurrentUser', JSON.stringify(user));
        
        // Feedback visivo "estremo" per confermare l'avvio
        const overlay = document.createElement('div');
        overlay.style.position = 'fixed';
        overlay.style.top = '0';
        overlay.style.left = '0';
        overlay.style.width = '100vw';
        overlay.style.height = '100vh';
        overlay.style.background = 'rgba(0,0,0,0.8)';
        overlay.style.zIndex = '9999';
        overlay.style.display = 'flex';
        overlay.style.alignItems = 'center';
        overlay.style.justifyContent = 'center';
        overlay.style.color = 'white';
        overlay.style.fontSize = '2rem';
        overlay.innerHTML = 'Cambio account in corso...';
        document.body.appendChild(overlay);
        
        console.log("Auth: Reload in corso...");
        setTimeout(() => {
            location.href = location.href; // Forza reload totale
        }, 150);
    }

    async function handleLogout() {
        console.log("Auth: Esecuzione logout...");
        if (currentUser) {
            let loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
            const idx = loggedUsers.findIndex(u => u.email === currentUser.email);
            if (idx >= 0) {
                delete loggedUsers[idx].session;
                localStorage.setItem('financeLoggedUsers', JSON.stringify(loggedUsers));
            }
            if (supabase) await supabase.auth.signOut();
            localStorage.removeItem('financeCurrentUser');
            const oldUserEmail = currentUser.email;
            currentUser = null;
            const nextActiveUser = loggedUsers.find(u => u.session && u.email !== oldUserEmail);
            if (nextActiveUser) {
                localStorage.setItem('financeCurrentUser', JSON.stringify(nextActiveUser));
            }
        }
        monthlyBudgets = {};
        events = {};
        familyMonthlyBudgets = {};
        familyEvents = {};
        lastInputValues = {};
        location.reload();
    }

    async function checkAuthStatus() {
        console.log("Auth: Checking status... Current user:", currentUser ? currentUser.email : "none");
        document.querySelectorAll('.cancel-auth-container').forEach(el => el.classList.add('hidden'));
        
        try {
            const accountSelector = document.getElementById('account-selector');
            const appContainer = document.getElementById('app-interface'); 
            const loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];

            if (currentUser) {
                // Initialize language from storage
                currentLanguage = localStorage.getItem('financeLanguage') || 'it';
                document.documentElement.lang = currentLanguage;
                applyTranslations(currentLanguage);

                // Add event listeners for language buttons
                document.querySelectorAll('.lang-btn').forEach(btn => {
                    const lang = btn.getAttribute('data-lang');
                    btn.classList.toggle('active', lang === currentLanguage);
                    btn.style.border = lang === currentLanguage ? '2px solid var(--primary)' : '2px solid transparent';
                    
                    btn.addEventListener('click', () => {
                        currentLanguage = lang;
                        changeLanguage(lang);
                    });
                });

                // --- SESSION VALIDATION & RESTORATION ---
                if (supabase && currentUser.id) {
                    console.log("Auth: Caricamento utente Supabase...");
                    const { data: { user }, error: userError } = await supabase.auth.getUser();
                    
                    if (userError || !user || user.id !== currentUser.id) {
                        if (currentUser.session && currentUser.session.access_token) {
                            console.log("Auth: Sessione non sincronizzata o mancante. Tentativo di ripristino per:", currentUser.email);
                            const { error: sessionError } = await supabase.auth.setSession({
                                access_token: currentUser.session.access_token,
                                refresh_token: currentUser.session.refresh_token
                            });
                            
                            if (sessionError) {
                                console.warn("Auth: Ripristino fallito per:", currentUser.email, sessionError.message);
                                
                                // Rimuoviamo la sessione non piÃ¹ valida dai loggedUsers per forzare ri-login
                                let loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
                                const idx = loggedUsers.findIndex(u => u.email === currentUser.email);
                                if (idx >= 0) {
                                    delete loggedUsers[idx].session;
                                    localStorage.setItem('financeLoggedUsers', JSON.stringify(loggedUsers));
                                }

                                // Invece di fare handleLogout (che farebbe switch), resettiamo solo l'utente attuale
                                localStorage.removeItem('financeCurrentUser');
                                currentUser = null;
                                location.reload();
                                return;
                            }
                            console.log("Auth: Sessione sincronizzata con successo.");
                        } else {
                            console.warn("Auth: Dati sessione locali mancanti. Reset utente.");
                            localStorage.removeItem('financeCurrentUser');
                            currentUser = null;
                            location.reload();
                            return;
                        }
                    } else {
                        console.log("Auth: Sessione Supabase già valida per", user.email);
                    }
                }

                if (authScreen) authScreen.classList.add('hidden');
                if (appContainer) appContainer.classList.remove('hidden');
                if (accountSelector) accountSelector.classList.add('hidden');
                if (currentUserNameDisplay) currentUserNameDisplay.textContent = currentUser.username;
                
                // Aggiorna carta virtuale in tutte le sezioni
                const cardUsernames = ['virtual-card-username-budget', 'virtual-card-username-settings'];
                cardUsernames.forEach(id => {
                    const el = document.getElementById(id);
                    if (el) el.textContent = currentUser.username;
                });

                updateLoggedUsersList();
                
                // --- SYNC INITIALIZATION ---
                monthlyBudgets = JSON.parse(localStorage.getItem(getStorageKey('financeMonthlyBudgets'))) || {};
                events = JSON.parse(localStorage.getItem(getStorageKey('financeEvents'))) || {};
                lastInputValues = {};

                const famId = localStorage.getItem(`${currentUser.email}_familyId`);
                if (famId) {
                    if (famId.startsWith('FAM-')) {
                        localStorage.removeItem(`${currentUser.email}_familyId`);
                        familyMonthlyBudgets = {};
                        familyEvents = {};
                    } else {
                        familyMonthlyBudgets = JSON.parse(localStorage.getItem(`${famId}_financeMonthlyBudgets`)) || {};
                        familyEvents = JSON.parse(localStorage.getItem(`${famId}_financeEvents`)) || {};
                    }
                } else {
                    familyMonthlyBudgets = {};
                    familyEvents = {};
                }

                // Carica lo sfondo personalizzato
                const savedBg = localStorage.getItem(getStorageKey('financeBackground'));
                if (typeof setBackground === 'function') {
                    setBackground(savedBg || 'bg-forest');
                }

                // Assicurati che ci sia un tema predefinito se non salvato
                if (!localStorage.getItem('financeTheme')) {
                    if (typeof setTheme === 'function') setTheme('default');
                }

                // Carica lo stile della carta
                const savedCardStyle = localStorage.getItem(getStorageKey('financeCardStyleClass')) || 'card-style-default';
                allVirtualCards.forEach(card => {
                    if (card.id !== 'virtual-card-family') {
                        card.classList.forEach(cls => { if(cls.startsWith('card-style-')) card.classList.remove(cls); });
                        card.classList.add(savedCardStyle);
                    }
                });

                const localPic = localStorage.getItem(getStorageKey('financeProfilePic'));
                updateAvatarUI(localPic); 
                loadCurrentMonthBudget();
                loadGoals();
                renderCalendar();
                initializeLastValues();
                if (typeof renderYearlyChart === 'function') renderYearlyChart(); 

                await loadUserData(); 

                // --- MOBILE PRIORITY: Default View ---
                // If on mobile, prioritize the Budget tab for immediate utility.
                if (window.innerWidth < 768) {
                    console.log("Mobile priority: Switching to Budget tab");
                    switchTab('tab-budget');
                } else {
                    switchTab('tab-home');
                }
            } else if (loggedUsers.length > 0) {
                // Se ci sono account salvati ma nessuno è attivo, mostra il selettore
                if (authScreen) authScreen.classList.remove('hidden');
                if (appContainer) appContainer.classList.add('hidden');
                if (accountSelector) {
                    accountSelector.classList.remove('hidden');
                    updateSavedAccountsUI();
                }
                toggleAuthForms(false);
                // Nascondi form login se c'è il selettore
                if (loginForm) loginForm.classList.add('hidden');
            } else {
                // Nessun account, mostra il login normale
                if (authScreen) authScreen.classList.remove('hidden');
                if (appContainer) appContainer.classList.add('hidden');
                if (accountSelector) accountSelector.classList.add('hidden');
                toggleAuthForms(false);
            }
            console.log("Auth: checkAuthStatus completed");
        } catch (err) {
            console.error("CRITICAL Error in checkAuthStatus:", err);
            // Non allertare l'utente per errori non bloccanti
        }
    }

    function updateSavedAccountsUI() {
        const list = document.getElementById('saved-accounts-list');
        if (!list) return;
        list.innerHTML = '';
        const loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];

        loggedUsers.forEach(user => {
            const item = document.createElement('div');
            item.className = 'account-item';
            
            const userPic = localStorage.getItem(`${user.email}_financeProfilePic`);
            const avatarHtml = userPic 
                ? `<img src="${userPic}" class="account-avatar">` 
                : `<div class="account-avatar"><i class="fa-solid fa-circle-user"></i></div>`;

            item.innerHTML = `
                ${avatarHtml}
                <div class="account-info">
                    <span class="account-name">${user.username}</span>
                    <span class="account-email">${user.email}</span>
                </div>
                <button class="delete-account-btn" title="Rimuovi account">
                    <i class="fa-solid fa-trash-can"></i>
                </button>
            `;

            // Click per accedere
            item.addEventListener('click', () => {
                if (user.session) {
                    switchAccount(user);
                } else {
                    // Se non c'è sessione (perché abbiamo fatto logout), mostriamo il form login pre-compilato
                    const loginEmailInput = document.getElementById('login-email');
                    const loginPasswordInput = document.getElementById('login-password');
                    const loginForm = document.getElementById('login-form');
                    const accountSelector = document.getElementById('account-selector');
                    
                    if (loginEmailInput) loginEmailInput.value = user.email;
                    if (loginPasswordInput) {
                        loginPasswordInput.value = '';
                        loginPasswordInput.focus();
                    }
                    
                    if (accountSelector) accountSelector.classList.add('hidden');
                    if (loginForm) loginForm.classList.remove('hidden');
                }
            });

            // Click per eliminare (non propaga al login)
            const delBtn = item.querySelector('.delete-account-btn');
            delBtn.addEventListener('click', async (e) => {
                e.stopPropagation();
                if (confirm(`Rimuovere l'account di ${user.username} da questo dispositivo?`)) {
                    let users = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
                    users = users.filter(u => u.email !== user.email);
                    localStorage.setItem('financeLoggedUsers', JSON.stringify(users));

                    // Se stiamo rimuovendo l'utente attualmente loggato, scolleghiamolo
                    if (currentUser && currentUser.email === user.email) {
                        localStorage.removeItem('financeCurrentUser');
                        currentUser = null;
                        if (supabase) await supabase.auth.signOut();
                        location.reload();
                    } else {
                        checkAuthStatus(); // Aggiorna UI se era un altro account
                    }
                }
            });

            list.appendChild(item);
        });
    }

    function updateLoggedUsersList() {
        const listContainer = document.getElementById('logged-users-list');
        if (!listContainer) return;
        listContainer.innerHTML = '';
        let loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];

        console.error("Auth: Aggiornamento lista utenti. Totale salvati:", loggedUsers.length);
        const activeUsers = loggedUsers.filter(user => {
            const isCurrent = currentUser && currentUser.email && user.email && 
                              currentUser.email.trim().toLowerCase() === user.email.trim().toLowerCase();
            return isCurrent || (user.session && user.session.access_token);
        });

        activeUsers.forEach(user => {
            const isCurrent = currentUser && currentUser.email && user.email && 
                              currentUser.email.trim().toLowerCase() === user.email.trim().toLowerCase();
            const userBtn = document.createElement('button');
            userBtn.type = 'button';
            userBtn.className = 'dropdown-item' + (isCurrent ? ' active' : '');
            
            userBtn.style.width = '100%';
            userBtn.style.display = 'flex';
            userBtn.style.alignItems = 'center';
            userBtn.style.gap = '12px';
            userBtn.style.padding = '12px 15px';
            userBtn.style.border = 'none';
            userBtn.style.background = isCurrent ? 'rgba(255, 255, 255, 0.08)' : 'transparent';
            userBtn.style.cursor = isCurrent ? 'default' : 'pointer';
            userBtn.style.color = 'var(--text-main)';
            userBtn.style.borderRadius = '8px';
            userBtn.style.marginBottom = '4px';
            userBtn.style.transition = 'background 0.2s';
            userBtn.style.position = 'relative';
            userBtn.style.zIndex = '5';
            userBtn.style.pointerEvents = 'auto';

            const userPic = localStorage.getItem(`${user.email}_financeProfilePic`);
            const avatarHtml = userPic 
                ? `<img src="${userPic}" class="user-avatar" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover; flex-shrink: 0; pointer-events: none;">` 
                : `<i class="fa-solid fa-circle-user" style="font-size: 28px; color: var(--text-muted); flex-shrink: 0; pointer-events: none;"></i>`;

            userBtn.innerHTML = `
                ${avatarHtml} 
                <div style="flex-grow: 1; text-align: left; display: flex; flex-direction: column; overflow: hidden; pointer-events: none;">
                    <span style="font-weight: 600; font-size: 0.95rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; pointer-events: none;">${user.username}</span>
                    <span style="font-size: 0.75rem; opacity: 0.6; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; pointer-events: none;">${user.email}</span>
                </div>
                ${isCurrent ? '<i class="fa-solid fa-check" style="color: var(--success); pointer-events: none;"></i>' : ''}
            `;

            if (!isCurrent) {
                // Marcatura per delegazione
                userBtn.classList.add('switch-target');
                userBtn.setAttribute('data-email', user.email);
            }
            
            listContainer.appendChild(userBtn);
        });

        // Delegazione evento sul container per massima affidabilità
        listContainer.onclick = (e) => {
            const btn = e.target.closest('.switch-target');
            if (btn) {
                const email = btn.getAttribute('data-email');
                console.error("Auth: DELEGAZIONE CLICK per:", email);
                const user = activeUsers.find(u => u.email === email);
                if (user) switchAccount(user);
            }
        };
    }

    // --- Auth Listeners ---
    if (showSignupLink) showSignupLink.addEventListener('click', (e) => { e.preventDefault(); toggleAuthForms(true); });
    if (showLoginLink) showLoginLink.addEventListener('click', (e) => { e.preventDefault(); toggleAuthForms(false); });
    if (signupBtn) signupBtn.addEventListener('click', handleSignup);
    if (loginBtn) loginBtn.addEventListener('click', handleLogin);

    // Gestione "Annulla" e "Usa un altro account"
    document.querySelectorAll('.cancel-auth-link').forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            const loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
            if (loggedUsers.length > 0) {
                // Se abbiamo account salvati, torna al selettore
                checkAuthStatus();
            } else {
                // Altrimenti (molto raro qui), resta al login
                toggleAuthForms(false);
            }
        });
    });

    const useAnotherBtn = document.getElementById('use-another-account-btn');
    if (useAnotherBtn) {
        useAnotherBtn.addEventListener('click', () => {
            document.getElementById('account-selector').classList.add('hidden');
            document.getElementById('login-form').classList.remove('hidden');
            document.querySelectorAll('.cancel-auth-container').forEach(el => el.classList.remove('hidden'));
        });
    }

    // NEW: User Menu & Dropdown logic
    const userMenuToggle = document.getElementById('user-menu-toggle');
    const userDropdown = document.getElementById('user-dropdown');
    const logoutBtnDropdown = document.getElementById('logout-btn-dropdown');
    const addAccountBtnDropdown = document.getElementById('add-account-btn-dropdown');

    if (userMenuToggle) {
        userMenuToggle.addEventListener('click', (e) => {
            e.stopPropagation();
            userDropdown.classList.toggle('hidden');
        });
    }

    // Ripristinato: Chiudi menù al click esterno
    document.addEventListener('click', (e) => {
        if (userDropdown && !userDropdown.classList.contains('hidden')) {
            if (!userMenuToggle.contains(e.target) && !userDropdown.contains(e.target)) {
                userDropdown.classList.add('hidden');
            }
        }
    });

    if (logoutBtnDropdown) {
        logoutBtnDropdown.addEventListener('click', handleLogout);
    }

    if (addAccountBtnDropdown) {
        addAccountBtnDropdown.addEventListener('click', async (e) => {
            e.stopPropagation();
            if (userDropdown) userDropdown.classList.add('hidden');
            
            // Per aggiungere un account mantenendo quello attuale "nella lista", 
            // ma rendendo quello nuovo "attivo", dobbiamo:
            // 1. NON fare signOut da Supabase (o potremmo invalidare Account A)
            // 2. Semplicemente mostrare il form di login "pulito"
            
            currentUser = null;
            localStorage.removeItem('financeCurrentUser');
            
            // Pulizia preventiva dei token Supabase per evitare conflitti con la nuova sessione
            Object.keys(localStorage).forEach(key => {
                if (key.startsWith('sb-') || key.includes('supabase-auth-token')) {
                    localStorage.removeItem(key);
                }
            });
            
            // Pulisci le variabili globali di stato UI prima del nuovo login
            monthlyBudgets = {};
            events = {};
            lastInputValues = {};
            
            // Svuota i campi UI visibili
            allBudgetInputs.forEach(input => { if (input) input.value = ''; });
            if (incomeInput) incomeInput.value = '';

            // Mostra la schermata auth
            if (appContainer) appContainer.classList.add('hidden');
            if (authScreen) authScreen.classList.remove('hidden');
            const accSelector = document.getElementById('account-selector');
            const loginFormEl = document.getElementById('login-form');
            if (accSelector) accSelector.classList.add('hidden');
            if (loginFormEl) {
                loginFormEl.classList.remove('hidden');
                // Reset form values
                document.getElementById('login-email').value = '';
                document.getElementById('login-password').value = '';
            }
            document.querySelectorAll('.cancel-auth-container').forEach(el => el.classList.remove('hidden'));
            
            toggleAuthForms(false);
        });
    }

    // --- Settings UI Interactions ---
    const settingsBtnDropdown = document.getElementById('settings-btn-dropdown');

    if (settingsBtnDropdown) {
        settingsBtnDropdown.addEventListener('click', async (e) => {
            e.stopPropagation();
            if (userDropdown) userDropdown.classList.add('hidden');

            // Salva la tab corrente prima di passare a settings
            const currentActiveTab = document.querySelector('.tab-pane.active');
            if (currentActiveTab && currentActiveTab.id !== 'tab-settings') {
                lastActiveTabId = currentActiveTab.id;
            }

            // Nascondi tutte le tab e disattiva i bottoni
            document.querySelectorAll('.tab-pane').forEach(pane => {
                pane.classList.remove('active');
                pane.classList.add('hidden');
            });
            document.querySelectorAll('.nav-btn').forEach(btn => btn.classList.remove('active'));

            // Mostra tab settings
            const tabSettings = document.getElementById('tab-settings');
            if (tabSettings) {
                tabSettings.classList.remove('hidden');
                tabSettings.classList.add('active');
            }

            // Attiva il pulsante nav corrispondente
            const settingsNavBtn = document.querySelector('.nav-btn[data-tab="tab-settings"]');
            if (settingsNavBtn) settingsNavBtn.classList.add('active');

            // Populate data
            const settingsUsernameInput = document.getElementById('settings-username');
            const settingsEmailInput = document.getElementById('settings-email');

            if (settingsUsernameInput && currentUser) {
                settingsUsernameInput.value = currentUser.username || '';
            }

            if (settingsEmailInput && currentUser) {
                if (currentUser.email) {
                    settingsEmailInput.value = currentUser.email;
                } else {
                    // local fallback
                    let users = JSON.parse(localStorage.getItem('financeUsers')) || [];
                    const localUser = users.find(u => u.username === currentUser.username);
                    settingsEmailInput.value = localUser ? (localUser.email || '') : '';

                    // try fetch from supabase session
                    if (supabase) {
                        const { data: { user } } = await supabase.auth.getUser();
                        if (user && user.email) {
                            settingsEmailInput.value = user.email;
                        } else if (user && user.user_metadata && user.user_metadata.real_email) {
                            settingsEmailInput.value = user.user_metadata.real_email;
                        }
                    }
                }
            }
        });
    }

    const saveSettingsBtn = document.getElementById('save-profile-btn');
    if (saveSettingsBtn) {
        saveSettingsBtn.addEventListener('click', async () => {
            const newUsername = document.getElementById('settings-username').value.trim();
            const settingsMsg = document.getElementById('settings-message');

            if (!newUsername) {
                settingsMsg.textContent = "L'username non può essere vuoto.";
                settingsMsg.style.color = "var(--danger)";
                return;
            }
             if (newUsername === currentUser.username) {
                settingsMsg.textContent = "Nessuna modifica rilevata.";
                settingsMsg.style.color = "var(--warning)";
                return;
            }

            let usernameChanged = newUsername !== currentUser.username;

            if (usernameChanged) {
                // 1. Check LocalStorage `financeUsers` to avoid duplicates
                let users = JSON.parse(localStorage.getItem('financeUsers')) || [];
                if (users.find(u => u.username === newUsername)) {
                    settingsMsg.textContent = "Questo username è già in uso.";
                    settingsMsg.style.color = "var(--danger)";
                    return;
                }

                const localUserIdx = users.findIndex(u => u.username === currentUser.username);
                if (localUserIdx >= 0) {
                    users[localUserIdx].username = newUsername;
                    localStorage.setItem('financeUsers', JSON.stringify(users));
                }

                // 2. Update Supabase if active
                if (supabase) {
                    const { data, error } = await supabase.auth.updateUser({
                        data: { username: newUsername }
                    });
                    if (error) {
                        console.error("Errore aggiornamento username su Supabase:", error);
                    }
                }

                // Update reference in logged users
                let loggedUsers = JSON.parse(localStorage.getItem('financeLoggedUsers')) || [];
                const lUserIdx = loggedUsers.findIndex(u => u.username === currentUser.username);
                if (lUserIdx >= 0) {
                    loggedUsers[lUserIdx].username = newUsername;
                    localStorage.setItem('financeLoggedUsers', JSON.stringify(loggedUsers));
                }

                // Migrate keys for data (they use username prefixes!)
                const oldPrefix = currentUser.username + '_';
                const newPrefix = newUsername + '_';

                const keysToMigrate = ['financeMonthlyBudgets', 'financeEvents', 'financeGoals', 'financeProfilePic'];
                keysToMigrate.forEach(k => {
                    const oldData = localStorage.getItem(oldPrefix + k);
                    if (oldData) {
                        localStorage.setItem(newPrefix + k, oldData);
                        localStorage.removeItem(oldPrefix + k);
                    }
                });

                // Update current user state
                currentUser.username = newUsername;
                localStorage.setItem('financeCurrentUser', JSON.stringify(currentUser));
                localStorage.setItem('financeLastUser', newUsername);
            }

            settingsMsg.textContent = usernameChanged ? "Username aggiornato con successo!" : "Modifiche salvate!";
            settingsMsg.style.color = "var(--success)";

            // Wait slightly and refresh if username changed
            if (usernameChanged) {
                setTimeout(() => {
                    location.reload();
                }, 1000);
            }
        });
    }

    // --- Real Profile Pic Selection Logic ---
    if (avatarPreviewTrigger && picSourceModal) {
        avatarPreviewTrigger.addEventListener('click', () => {
            picSourceModal.classList.remove('hidden');
        });
    }




    if (closePicSourceModalBtn && picSourceModal) {
        closePicSourceModalBtn.addEventListener('click', () => {
            picSourceModal.classList.add('hidden');
        });
    }

    if (changePicGalleryBtn && profilePicInput) {
        changePicGalleryBtn.addEventListener('click', () => {
            picSourceModal.classList.add('hidden');
            profilePicInput.click();
        });

        profilePicInput.addEventListener('change', (e) => {
            const file = e.target.files[0];
            if (file) {
                if (file.size > 1024 * 1024) {
                    alert("L'immagine è troppo grande. Scegli un file inferiore a 1MB.");
                    return;
                }
                const reader = new FileReader();
                reader.onload = (event) => {
                    const base64 = event.target.result;
                    updateAvatarUI(base64, true);
                };
                reader.readAsDataURL(file);
            }
        });
    }

    // Camera Modal Logic
    if (changePicCameraBtn && cameraModal && cameraVideo && captureBtn) {
        changePicCameraBtn.addEventListener('click', async () => {
            if (picSourceModal) picSourceModal.classList.add('hidden');
            try {
                cameraStream = await navigator.mediaDevices.getUserMedia({
                    video: { facingMode: "user" },
                    audio: false
                });
                cameraVideo.srcObject = cameraStream;
                cameraModal.classList.remove('hidden');
            } catch (err) {
                console.error("Errore accesso fotocamera:", err);
                alert("Impossibile accedere alla fotocamera. Assicurati di aver dato i permessi.");
            }
        });

        const stopCamera = () => {
            if (cameraStream) {
                cameraStream.getTracks().forEach(track => track.stop());
                cameraStream = null;
            }
            cameraModal.classList.add('hidden');
        };

        if (closeCameraBtn) closeCameraBtn.addEventListener('click', stopCamera);

        captureBtn.addEventListener('click', () => {
            if (!cameraVideo || !cameraCanvas) return;
            const context = cameraCanvas.getContext('2d');
            cameraCanvas.width = cameraVideo.videoWidth;
            cameraCanvas.height = cameraVideo.videoHeight;
            
            // Draw mirrored image if needed (matching CSS transform)
            context.translate(cameraCanvas.width, 0);
            context.scale(-1, 1);
            context.drawImage(cameraVideo, 0, 0, cameraCanvas.width, cameraCanvas.height);
            
            const base64 = cameraCanvas.toDataURL('image/jpeg', 0.8);
            updateAvatarUI(base64, true);
            stopCamera();
        });
    }

    document.querySelectorAll('.cancel-auth-link').forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            if (currentUser) {
                checkAuthStatus();
            }
        });
    });



    // --- Saving Goals Logic ---
    const addGoalBtn = document.getElementById('add-goal-btn');
    const goalNameInput = document.getElementById('goal-name');
    const goalAmountInput = document.getElementById('goal-amount');
    const goalMonthsInput = document.getElementById('goal-months');
    const goalsContainer = document.getElementById('active-goals-grid');

    function loadGoals() {
        if (!currentUser) return;
        const goals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals')) || '[]');
        renderGoals(goals);
    }

    function renderGoals(goals) {
        goalsContainer.innerHTML = '';

        // Get current monthly savings (before goal deductions to show feasibility)
        const income = parseFloat(incomeInput.value) || 0;
        const rent = parseFloat(rentInput.value) || 0;
        const bills = parseFloat(billsInput.value) || 0;
        const auto = parseFloat(autoInput.value) || 0;
        const food = parseFloat(foodInput.value) || 0;
        const subs = parseFloat(subsInput.value) || 0;
        const leisure = parseFloat(leisureInput.value) || 0;
        const shopping = parseFloat(shoppingInput.value) || 0;
        const extra = parseFloat(extraInput.value) || 0;
        const baseExpenses = rent + bills + auto + food + subs + leisure + shopping + extra;
        const availableForGoals = income - baseExpenses;

        goals.forEach(goal => {
            const isCompleted = goal.isCompleted || false;
            const deposit = parseFloat(goal.deposit) || 0;
            const progressPercent = Math.min(100, Math.floor((goal.savedAmount / goal.amount) * 100) || 0);

            // Prediction based on set deposit
            const monthsLeft = deposit > 0 ? Math.ceil((goal.amount - (goal.savedAmount || 0)) / deposit) : '∞';

            let statusClass = 'status-on-track';
            let statusText = 'In linea';

            if (!isCompleted && deposit > availableForGoals) {
                statusClass = 'status-behind';
                statusText = 'Budget Insufficiente';
            }

            const goalCard = document.createElement('div');
            goalCard.className = `goal-card glass-panel ${isCompleted ? 'completed' : ''}`;
            goalCard.onclick = (e) => {
                if (e.target.tagName !== 'BUTTON' && !e.target.closest('button')) {
                    depositToGoal(goal.id);
                }
            };

            const radius = 36;
            const circumference = 2 * Math.PI * radius;
            const strokeDashoffset = circumference - (progressPercent / 100) * circumference;

            goalCard.innerHTML = `
                <button class="delete-goal" onclick="deleteGoal('${goal.id}')">
                    <i class="fa-solid fa-trash"></i>
                </button>
                <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                    <div style="flex-grow: 1;">
                        <h3 style="margin-bottom: 0.2rem;">${goal.name}</h3>
                        ${isCompleted ? '<span class="status-badge status-on-track">Completata</span>' : `<span class="status-badge ${statusClass}">${statusText}</span>`}
                        <div style="margin-top: 1rem; color: var(--text-muted); font-size: 0.9rem;">
                            <span>${currency.format(goal.savedAmount || 0)} / ${currency.format(goal.amount)}</span>
                        </div>
                    </div>
                    
                    <div class="circular-progress" style="position: relative; width: 85px; height: 85px; flex-shrink: 0; margin-left: 1rem;">
                        <svg width="85" height="85" viewBox="0 0 85 85" style="transform: rotate(-90deg);">
                            <circle cx="42.5" cy="42.5" r="${radius}" fill="none" stroke="var(--panel-border)" stroke-width="7"></circle>
                            <circle cx="42.5" cy="42.5" r="${radius}" fill="none" stroke="var(--primary)" stroke-width="7" 
                                stroke-dasharray="${circumference}" stroke-dashoffset="${strokeDashoffset}" 
                                stroke-linecap="round" style="transition: stroke-dashoffset 1s ease-in-out;"></circle>
                        </svg>
                        <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 1rem; color: var(--text-main);">
                            ${progressPercent}%
                        </div>
                    </div>
                </div>
                
                <div class="goal-prediction" style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid var(--panel-border);">
                    <div class="prediction-item" style="display: flex; align-items: center; gap: 8px; margin-bottom: 0.5rem;">
                        <i class="fa-solid fa-piggy-bank" style="color: var(--primary);"></i>
                        <span style="font-size: 0.9rem;">Mensile: <b class="editable-deposit" onclick="editGoalDeposit('${goal.id}')" style="cursor:pointer; color: var(--primary);">${currency.format(deposit)}</b></span>
                    </div>
                    <div class="prediction-item" style="display: flex; align-items: center; gap: 8px; margin-bottom: 1rem;">
                        <i class="fa-solid fa-clock" style="color: var(--text-muted);"></i>
                        <span style="font-size: 0.9rem;">Tempo: <b>${isCompleted ? 'Traguardo raggiunto!' : (monthsLeft + ' mesi')}</b></span>
                    </div>
                    
                    <div style="display: flex; gap: 0.5rem; margin-top: 0.5rem;">
                        ${goal.isPaid ? 
                            '<span class="status-badge status-on-track" style="background: var(--success); color: white; width: 100%; text-align: center; padding: 0.8rem;">Pagato</span>' : 
                            `<button class="primary-btn" style="flex: 2; padding: 0.8rem; font-size: 0.9rem; background: ${isCompleted ? 'var(--success)' : 'var(--primary)'};" onclick="depositToGoal('${goal.id}')">
                                ${isCompleted ? '<i class="fa-solid fa-cart-shopping"></i> Paga Ora' : '<i class="fa-solid fa-plus"></i> Deposita'}
                             </button>
                             <button class="primary-btn" style="flex: 1; padding: 0.8rem; font-size: 0.9rem; background: var(--panel-bg); color: var(--text-main); border: 1px solid var(--panel-border);" onclick="toggleGoalComplete('${goal.id}')">
                                <i class="fa-solid fa-check-double"></i>
                             </button>`
                        }
                    </div>
                </div>
            `;
            goalsContainer.appendChild(goalCard);
        });
    }

    window.depositToGoal = function (id) {
        const goals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals')) || '[]');
        const goal = goals.find(g => g.id === id);
        if (!goal || goal.isPaid) return;

        if (goal.isCompleted) {
            // SHOW PAYMENT MODAL for final purchase
            selectedGoalId = id;
            selectedPaymentMethod = null;
            paymentMethodBtns.forEach(b => b.classList.remove('selected'));
            depositAmountInput.value = goal.amount;
            depositAmountInput.disabled = true; 
            const modalTitle = depositModal.querySelector('h3');
            if (modalTitle) modalTitle.innerHTML = `<i class="fa-solid fa-cart-shopping"></i> Acquisto: ${goal.name}`;
            if (confirmDepositBtn) confirmDepositBtn.innerHTML = `<i class="fa-solid fa-check"></i> Conferma Acquisto`;
            if (depositModal) depositModal.classList.remove('hidden');
        } else {
            // DIRECT DEPOSIT via prompt
            const amountStr = prompt(`Quanto vuoi depositare oggi per "${goal.name}"?`, goal.deposit || 0);
            if (amountStr !== null) {
                const amount = parseFloat(amountStr);
                if (!isNaN(amount) && amount > 0) {
                    processGoalDeposit(goal, amount);
                } else if (amountStr !== "") {
                    alert("Inserisci un importo valido.");
                }
            }
        }
    };

    async function processGoalDeposit(goal, amount) {
        const goals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals')) || '[]');
        const targetGoal = goals.find(g => g.id === goal.id);
        if (!targetGoal) return;

        // 1. Update Goal
        targetGoal.savedAmount = (parseFloat(targetGoal.savedAmount) || 0) + amount;
        if (targetGoal.savedAmount >= targetGoal.amount) {
            targetGoal.savedAmount = targetGoal.amount;
            targetGoal.isCompleted = true;
        }

        // 2. Update Budget
        const currentExtra = parseFloat(extraInput.value) || 0;
        extraInput.value = (currentExtra + amount).toFixed(2);
        await saveCurrentMonthBudget(); 

        // 3. Add to Calendar (solo se è un deposito)
        const today = new Date();
        const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
        if (!events[dateStr]) events[dateStr] = [];
        events[dateStr].push({
            category: 'extra',
            amount: amount,
            isGoal: true,
            goalName: targetGoal.name,
            isFinalPurchase: false,
            addedBy: currentUser.username
        });

        // Persist
        localStorage.setItem(getStorageKey('financeGoals'), JSON.stringify(goals));
        localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
        await syncData('goals', goals);
        await syncData('events', events);

        // Refresh
        loadGoals();
        recalculateSummary();
        renderCalendar();
        initializeLastValues();

        if (targetGoal.isCompleted) {
            alert(`Obiettivo raggiunto! Ora puoi procedere al pagamento finale per: ${targetGoal.name}`);
        }
    }

    if (paymentMethodBtns) {
        paymentMethodBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                paymentMethodBtns.forEach(b => b.classList.remove('selected'));
                btn.classList.add('selected');
                selectedPaymentMethod = btn.getAttribute('data-method');
            });
        });
    }

    if (confirmDepositBtn) {
        confirmDepositBtn.addEventListener('click', async () => {
            if (!selectedGoalId) return;
            const amount = parseFloat(depositAmountInput.value);
            if (isNaN(amount) || amount <= 0) {
                alert("Inserisci un importo valido.");
                return;
            }
            if (!selectedPaymentMethod) {
                alert("Seleziona un metodo di pagamento.");
                return;
            }

            const goals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals')) || '[]');
            const goal = goals.find(g => g.id === selectedGoalId);
            if (!goal) return;

            const isFinalPurchase = goal.isCompleted;

            // 1. Update Goal
            if (isFinalPurchase) {
                goal.isPaid = true;
            } else {
                goal.savedAmount = (parseFloat(goal.savedAmount) || 0) + amount;
                if (goal.savedAmount >= goal.amount) {
                    goal.savedAmount = goal.amount;
                    goal.isCompleted = true;
                }
            }

            // 2. Budget Scaling (Not for final purchase if funds were already saved)
            if (!isFinalPurchase) {
                const currentExtra = parseFloat(extraInput.value) || 0;
                extraInput.value = (currentExtra + amount).toFixed(2);
                await saveCurrentMonthBudget(); 
                
                // 3. Add to Calendar (solo se è un deposito)
                const today = new Date();
                const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
                if (!events[dateStr]) events[dateStr] = [];
                events[dateStr].push({
                    category: 'extra',
                    amount: amount,
                    isGoal: true,
                    goalName: goal.name,
                    isFinalPurchase: false,
                    paymentMethod: selectedPaymentMethod,
                    addedBy: currentUser.username
                });
            } else {
                // Pagamento finale: i soldi vengono presi dall'obiettivo stesso (già depositati in passato)
                // Non aggiungiamo nulla alle spese extra e possiamo al massimo segnare un evento nel calendario di acquisto
                const today = new Date();
                const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
                if (!events[dateStr]) events[dateStr] = [];
                // Lo mettiamo come 'other' o categoria neutra senza scalare il main budget
                 events[dateStr].push({
                    category: 'other',
                    amount: amount, // Mostra l'importo pagato dall'obiettivo
                    isGoal: true,
                    goalName: goal.name,
                    isFinalPurchase: true,
                    paymentMethod: selectedPaymentMethod,
                    addedBy: currentUser.username
                });
            }

            // Persist
            localStorage.setItem(getStorageKey('financeGoals'), JSON.stringify(goals));
            localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
            await syncData('goals', goals);
            await syncData('events', events);

            // Refresh
            if (depositModal) depositModal.classList.add('hidden');
            loadGoals();
            recalculateSummary();
            renderCalendar();
            initializeLastValues();

            if (isFinalPurchase) {
                alert(`Pagamento completato per: ${goal.name}. Ora l'obiettivo è segnato come pagato.`);
            } else if (goal.isCompleted) {
                alert(`Obiettivo raggiunto! Ora puoi procedere al pagamento finale per: ${goal.name}`);
            }
        });
    }

    if (closeDepositModalBtn) {
        closeDepositModalBtn.addEventListener('click', () => {
            if (depositModal) depositModal.classList.add('hidden');
        });
    }

    window.editGoalDeposit = async function (id) {
        const goals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals')) || '[]');
        const goal = goals.find(g => g.id === id);
        if (!goal) return;

        const newDeposit = prompt(`Quanto vuoi depositare mensilmente per "${goal.name}"?`, goal.deposit);
        if (newDeposit !== null) {
            const val = parseFloat(newDeposit);
            if (!isNaN(val) && val >= 0) {
                goal.deposit = val;
                localStorage.setItem(getStorageKey('financeGoals'), JSON.stringify(goals));
                await syncData('goals', goals);
                loadGoals();
                recalculateSummary();
            } else {
                alert("Inserisci un importo valido.");
            }
        }
    };

    window.toggleGoalComplete = async function (id) {
        const goals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals')) || '[]');
        const goal = goals.find(g => g.id === id);
        if (!goal) return;

        goal.isCompleted = !goal.isCompleted;
        if (goal.isCompleted) {
            goal.savedAmount = goal.amount; // Mark as fully saved
        } else {
            goal.savedAmount = 0; // Reset or keep partial? Let's reset for simplicity
        }

        localStorage.setItem(getStorageKey('financeGoals'), JSON.stringify(goals));
        await syncData('goals', goals);
        loadGoals();
        recalculateSummary();
    };

    async function addGoal() {
        const name = goalNameInput.value.trim();
        const amount = parseFloat(goalAmountInput.value);
        const months = parseInt(goalMonthsInput.value);
        const depositInput = document.getElementById('goal-deposit');
        const deposit = parseFloat(depositInput.value) || 0;

        if (!name || isNaN(amount) || isNaN(months) || amount <= 0 || months <= 0) {
            alert('Per favore inserisci dati validi per l\'obiettivo.');
            return;
        }

        const goals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals')) || '[]');
        const newGoal = {
            id: Date.now().toString(),
            name,
            amount,
            months,
            deposit,
            savedAmount: 0,
            isCompleted: false,
            createdAt: new Date().toISOString()
        };

        goals.push(newGoal);
        localStorage.setItem(getStorageKey('financeGoals'), JSON.stringify(goals));
        await syncData('goals', goals);

        goalNameInput.value = '';
        goalAmountInput.value = '';
        goalMonthsInput.value = '';
        depositInput.value = '';

        loadGoals();
        recalculateSummary();
    }

    // --- Auto-calculation for Goal Deposit ---
    function updateSuggestedDeposit() {
        const amount = parseFloat(goalAmountInput.value) || 0;
        const months = parseInt(goalMonthsInput.value) || 0;
        const depositInput = document.getElementById('goal-deposit');

        if (amount > 0 && months > 0) {
            const suggested = (amount / months).toFixed(2);
            depositInput.value = suggested;
        }
    }

    if (goalAmountInput) goalAmountInput.addEventListener('input', updateSuggestedDeposit);
    if (goalMonthsInput) goalMonthsInput.addEventListener('input', updateSuggestedDeposit);

    // Expose deleteGoal to global scope for onclick
    window.deleteGoal = async function (id) {
        if (!confirm('Eliminare questo obiettivo?')) return;
        let goals = JSON.parse(localStorage.getItem(getStorageKey('financeGoals')) || '[]');
        goals = goals.filter(g => g.id !== id);
        localStorage.setItem(getStorageKey('financeGoals'), JSON.stringify(goals));
        await syncData('goals', goals);
        loadGoals();
    };

    if (addGoalBtn) {
        addGoalBtn.addEventListener('click', addGoal);
    }



    // Load saved theme
    const savedTheme = localStorage.getItem('financeTheme') || 'default';
    setTheme(savedTheme);

    function setTheme(theme) {
        document.documentElement.setAttribute('data-theme', theme);
        localStorage.setItem('financeTheme', theme);
        if (currentUser) syncData('theme', theme);
        document.querySelectorAll('.theme-btn').forEach(btn => {
            btn.classList.toggle('active', btn.getAttribute('data-theme') === theme);
        });
    }

    document.querySelectorAll('.theme-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            setTheme(btn.getAttribute('data-theme'));
        });
    });

    // Background Selection Logic
    function setBackground(bgName) {
        if (!bgName) bgName = 'bg-forest';
        
        // Rimuove tutte le possibili classi di sfondo
        document.body.classList.remove('bg-forest', 'bg-ocean', 'bg-amethyst', 'bg-obsidian', 'bg-nebula', 'bg-aurora', 'bg-sunset', 'bg-frost', 'bg-royal', 'bg-cyber');
        // Applica quella selezionata
        document.body.classList.add(bgName);
        
        // Aggiorna l'interfaccia nelle impostazioni
        document.querySelectorAll('.bg-option').forEach(opt => {
            opt.classList.toggle('active', opt.getAttribute('data-bg') === bgName);
            // Sincronizza lo stile del bordo (fallback inline se non preso da CSS)
            opt.style.borderColor = opt.getAttribute('data-bg') === bgName ? 'var(--primary)' : 'transparent';
        });
        
        // Salva la preferenza dell'utente
        if (currentUser) {
            localStorage.setItem(getStorageKey('financeBackground'), bgName);
            syncData('background', bgName);
        }
    }

    document.querySelectorAll('.bg-option').forEach(opt => {
        opt.addEventListener('click', () => {
            setBackground(opt.getAttribute('data-bg'));
        });
    });

    // Card Style Selection Logic
    window.setCardStyle = function(styleName) {
        if (!styleName) styleName = 'card-style-default';
        
        const cardClasses = ['card-style-default', 'card-style-ocean', 'card-style-sand', 'card-style-gold', 'card-style-purple', 'card-style-dark'];
        
        const budgetCard = document.getElementById('virtual-card-budget');
        const previewCard = document.getElementById('settings-card-preview');

        if (budgetCard) {
            budgetCard.classList.remove(...cardClasses);
            budgetCard.classList.add(styleName);
        }
        if (previewCard) {
            previewCard.classList.remove(...cardClasses);
            previewCard.classList.add(styleName);
        }
        
        // Aggiorna l'interfaccia nelle impostazioni
        document.querySelectorAll('.style-option').forEach(opt => {
            opt.classList.toggle('active', opt.getAttribute('data-style') === styleName);
            opt.style.borderColor = opt.getAttribute('data-style') === styleName ? 'var(--primary)' : 'transparent';
        });
        
        // Salva la preferenza dell'utente
        if (currentUser) {
            localStorage.setItem(getStorageKey('financeCardStyle'), styleName);
        }
    };

    document.querySelectorAll('.style-option').forEach(opt => {
        opt.addEventListener('click', () => {
            setCardStyle(opt.getAttribute('data-style'));
        });
    });

    // Password Toggle Listeners
    document.querySelectorAll('.password-toggle').forEach(button => {
        button.addEventListener('click', () => {
            const targetId = button.getAttribute('data-target');
            const input = document.getElementById(targetId);
            const icon = button.querySelector('i');

            if (input.type === 'password') {
                input.type = 'text';
                icon.classList.remove('fa-eye');
                icon.classList.add('fa-eye-slash');
            } else {
                input.type = 'password';
                icon.classList.remove('fa-eye-slash');
                icon.classList.add('fa-eye');
            }
        });
    });



    async function saveCurrentMonthBudget() {
        if (!currentDate) return;
        const year = currentDate.getFullYear();
        const month = currentDate.getMonth(); // 0-11
        const key = `${year}-${String(month + 1).padStart(2, '0')}`;

        monthlyBudgets[key] = {
            income: parseFloat(incomeInput.value) || 0,
            rent: parseFloat(rentInput.value) || 0,
            bills: parseFloat(billsInput.value) || 0,
            auto: parseFloat(autoInput.value) || 0,
            food: parseFloat(foodInput.value) || 0,
            subs: parseFloat(subsInput.value) || 0,
            leisure: parseFloat(leisureInput.value) || 0,
            shopping: parseFloat(shoppingInput.value) || 0,
            extra: parseFloat(extraInput.value) || 0
        };

        localStorage.setItem(getStorageKey('financeMonthlyBudgets'), JSON.stringify(monthlyBudgets));
        await syncData('budgets', monthlyBudgets);
    }

    // Formatter per valuta
    async function loadCurrentMonthBudget() {
        if (!currentDate) return;
        const year = currentDate.getFullYear();
        const month = currentDate.getMonth();
        const key = `${year}-${String(month + 1).padStart(2, '0')}`;

        let budget = monthlyBudgets[key] || { income: '', rent: '', bills: '', auto: '', food: '', subs: '', leisure: '', shopping: '', extra: '' };



        // Se 0 nel salvataggio, lo mostriamo vuoto per pulizia a meno che non fosse intenzionale, ma lasciamo il fallback a stringa vuota per i mesi nuovi
        incomeInput.value = budget.income || '';
        rentInput.value = budget.rent || '';
        billsInput.value = budget.bills || '';
        autoInput.value = budget.auto || '';
        foodInput.value = budget.food || '';
        subsInput.value = budget.subs || '';
        leisureInput.value = budget.leisure || '';
        shoppingInput.value = budget.shopping || '';
        extraInput.value = budget.extra || '';

        // Mostra il summary se ci sono dati caricati
        recalculateSummary();
    }

    // Salva automaticamente input manuali e implementa logica ADDITIVA
    allBudgetInputs.forEach(input => {
        if (input) {
            // Ricalcolo in tempo reale per lo sguardo veloce
            input.addEventListener('input', recalculateSummary);

            input.addEventListener('change', async () => {
                const enteredVal = parseFloat(input.value) || 0;

                // --- Logica Standard: Il valore inserito sostituisce il precedente ---
                input.value = enteredVal;
                lastInputValues[input.id] = enteredVal;

                // --- Magic Sync: Automatic Event Recording ---
                if (enteredVal !== 0) {
                    const today = new Date();
                    const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;

                    if (!events[dateStr]) events[dateStr] = [];

                    events[dateStr].push({
                        category: input.id,
                        amount: Math.abs(enteredVal),
                        addedBy: currentUser.username
                    });

                    localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
                    await syncData('events', events);
                    renderCalendar();
                }

                await saveCurrentMonthBudget();
                recalculateSummary();
            });
        }
    });

    // --- Data Deletion Logic for Budget Inputs ---
    document.querySelectorAll('.clear-input-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            const targetId = btn.getAttribute('data-target');
            const targetInput = document.getElementById(targetId);
            if (targetInput) {
                targetInput.value = '';
                // Trigger event change per salvare e ricalcolare
                targetInput.dispatchEvent(new Event('change'));

                // Rimuovi anche gli eventi per la data corrispondente a oggi 
                // in modo che cliccando la X si elimini anche la registrazione nel calendario
                const today = new Date();
                const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;

                if (events[dateStr]) {
                    events[dateStr] = events[dateStr].filter(e => e.category !== targetId);
                    if (events[dateStr].length === 0) {
                        delete events[dateStr];
                    }
                    localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
                    await syncData('events', events);
                    renderCalendar();
                }
            }
        });
    });


    const monthNames = [
        "Gennaio", "Febbraio", "Marzo", "Aprile", "Maggio", "Giugno",
        "Luglio", "Agosto", "Settembre", "Ottobre", "Novembre", "Dicembre"
    ];





    function renderCalendar() {
        if (!calendarGrid) {
            console.error("Calendar grid not found!");
            return;
        }
        calendarGrid.innerHTML = '';
        const year = currentDate.getFullYear();
        const month = currentDate.getMonth();

        if (calendarMonthYear) calendarMonthYear.textContent = `${monthNames[month]} ${year}`;

        const firstDayOfMonth = new Date(year, month, 1).getDay();
        const daysInMonth = new Date(year, month + 1, 0).getDate();

        // Lunedì come primo giorno (0=Domenica -> diventa 6, altrimenti meno 1)
        const startingDay = firstDayOfMonth === 0 ? 6 : firstDayOfMonth - 1;

        // Slot vuoti
        for (let i = 0; i < startingDay; i++) {
            const emptyDiv = document.createElement('div');
            emptyDiv.className = 'calendar-day empty';
            calendarGrid.appendChild(emptyDiv);
        }

        const today = new Date();

        // Giorni
        for (let i = 1; i <= daysInMonth; i++) {
            const dayDiv = document.createElement('div');
            dayDiv.className = 'calendar-day';

            if (year === today.getFullYear() && month === today.getMonth() && i === today.getDate()) {
                dayDiv.classList.add('today');
            }

            const dateNum = document.createElement('span');
            dateNum.className = 'date-num';
            dateNum.textContent = i;
            dayDiv.appendChild(dateNum);

            // Mostra eventi sulla griglia del calendario
            const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(i).padStart(2, '0')}`;
            if (events[dateStr]) {
                events[dateStr].forEach((evt, index) => {
                    const evtDiv = document.createElement('div');
                    evtDiv.className = `event-indicator event-${categoryToType[evt.category] || 'other'}`;

                    let amountStr = '';
                    if (evt.amount) {
                        const type = categoryToType[evt.category] || 'other';
                        const prefix = type === 'expense' ? '-' : (type === 'income' ? '+' : '');
                        amountStr = ` ${prefix}€${evt.amount}`;
                    }

                    let titleText = getCategoryName(evt.category) || 'Altro';
                    if (evt.isGoal) {
                        titleText = evt.isFinalPurchase ? 'Acquisto Obiettivo' : 'Versamento Obiettivo';
                    }
                    evtDiv.textContent = `${titleText}${amountStr}`;

                    // Impediamo il click event bubble dalla griglia per far aprire sempre la Dettagli Modal per uniformità
                    evtDiv.addEventListener('click', (e) => {
                        e.stopPropagation();
                        openDayDetailsModal(dateStr, i, month, year);
                    });

                    dayDiv.appendChild(evtDiv);
                });
            }

            // Cliccando sul giorno intero si apre il riepilogo
            dayDiv.addEventListener('click', () => {
                openDayDetailsModal(dateStr, i, month, year);
            });

            calendarGrid.appendChild(dayDiv);
        }
    }

    function openDayDetailsModal(dateStr, day, month, year) {
        selectedDateInfo = dateStr;
        if (!dayDetailsModal) return;

        // Determina contesto
        const familyId = localStorage.getItem(`${currentUser.email}_familyId`);
        const isFamilyModal = (accountContext === 'family' && familyId);
        const targetEventsModal = isFamilyModal ? familyEvents : events;

        // Set title
        if (dayDetailsTitle) {
            dayDetailsTitle.textContent = `Eventi del ${day} ${monthNames[month]} ${year}`;
        }

        // Render Events
        if (dayEventsList) {
            dayEventsList.innerHTML = '';
            const dayEvents = targetEventsModal[dateStr] || [];

            if (dayEvents.length === 0) {
                dayEventsList.innerHTML = '<p style="color: var(--text-muted); font-size: 0.9rem; margin-top: 10px;">Nessun evento registrato.</p>';
            } else {
                dayEvents.forEach((evt, index) => {
                    const evtItem = document.createElement('div');
                    evtItem.style.display = 'flex';
                    evtItem.style.justifyContent = 'space-between';
                    evtItem.style.alignItems = 'center';
                    evtItem.style.padding = '10px';
                    evtItem.style.borderBottom = '1px solid var(--panel-border)';
                    evtItem.style.marginBottom = '5px';

                    let amountStr = '';
                    if (evt.amount) {
                        const type = categoryToType[evt.category] || 'other';
                        const prefix = type === 'expense' ? '-' : (type === 'income' ? '+' : '');
                        amountStr = ` ${prefix}€${evt.amount}`;
                    }

                    let titleText = categoryNames[evt.category] || evt.category;
                    const paymentStr = evt.paymentType ? ` (${evt.paymentType})` : '';
                    if (evt.isGoal) {
                        titleText = evt.isFinalPurchase ? `Acquisto: ${evt.goalName || ''}${paymentStr}` : `Versamento: ${evt.goalName || ''}${paymentStr}`;
                    }

                    const attributionHtml = (familyId && evt.addedBy) ? `<div class="event-attribution"><i class="fa-solid fa-user-tag"></i> Aggiunto da: ${evt.addedBy}</div>` : '';

                    evtItem.innerHTML = `
                        <div style="flex-grow: 1;">
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <span class="event-indicator event-${categoryToType[evt.category] || 'other'}">${titleText}</span>
                                <span style="font-weight: 500;">${amountStr}</span>
                            </div>
                            ${attributionHtml}
                        </div>
                        <button class="icon-btn" aria-label="Elimina Evento" style="color: var(--danger);"><i class="fa-solid fa-trash"></i></button>
                    `;

                    // Eliminazione Singola
                    const deleteBtn = evtItem.querySelector('button');
                    deleteBtn.addEventListener('click', async () => {
                        if (confirm(`Vuoi rimuovere l'evento "${titleText}"?`)) {
                            // Sincronizza con il budget: sottrai l'importo
                            if (evt.amount) {
                                const prefix = isFamilyModal ? 'f-' : '';
                                const targetInput = document.getElementById(prefix + evt.category);
                                if (targetInput) {
                                    const currentVal = parseFloat(targetInput.value) || 0;
                                    targetInput.value = Math.max(0, currentVal - parseFloat(evt.amount));
                                    
                                    if (isFamilyModal) {
                                        await saveFamilyBudget();
                                    } else {
                                        await saveCurrentMonthBudget();
                                        recalculateSummary();
                                        initializeLastValues();
                                    }
                                }
                            }

                            targetEventsModal[dateStr].splice(index, 1);
                            if (targetEventsModal[dateStr].length === 0) delete targetEventsModal[dateStr];
                            
                            if (isFamilyModal) {
                                localStorage.setItem(`${familyId}_financeEvents`, JSON.stringify(familyEvents));
                                await syncData('family_events', familyEvents, familyId);
                                renderFamilyCalendar();
                                updateRecentMovementsUI();
                            } else {
                                localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
                                await syncData('events', events); 
                                renderCalendar();
                            }
                            openDayDetailsModal(dateStr, day, month, year); // Refresh della vista
                        }
                    });

                    dayEventsList.appendChild(evtItem);
                });
            }
        }

        dayDetailsModal.classList.remove('hidden');
    }

    if (prevMonthBtn) {
        prevMonthBtn.addEventListener('click', async () => {
            await saveCurrentMonthBudget();
            currentDate.setMonth(currentDate.getMonth() - 1);
            await loadCurrentMonthBudget();
            renderCalendar();
        });
    }

    if (nextMonthBtn) {
        nextMonthBtn.addEventListener('click', async () => {
            await saveCurrentMonthBudget();
            currentDate.setMonth(currentDate.getMonth() + 1);
            await loadCurrentMonthBudget();
            renderCalendar();
        });
    }

    // --- Day Details Modal Actions ---
    if (closeDetailsModalBtn) {
        closeDetailsModalBtn.addEventListener('click', () => {
            if (dayDetailsModal) dayDetailsModal.classList.add('hidden');
        });
    }

    if (openAddEventBtn) {
        openAddEventBtn.addEventListener('click', () => {
            if (dayDetailsModal) dayDetailsModal.classList.add('hidden');
            if (eventModal) {
                eventModal.classList.remove('hidden');
                if (eventAmountInput) {
                    eventAmountInput.value = '';
                    eventAmountInput.focus();
                }
            }
        });
    }

        if (deleteAllEventsBtn) {
        deleteAllEventsBtn.addEventListener('click', async () => {
            const familyId = localStorage.getItem(`${currentUser.email}_familyId`);
            const isFamily = (accountContext === 'family' && familyId);
            const targetEvents = isFamily ? familyEvents : events;

            if (selectedDateInfo && targetEvents[selectedDateInfo]) {
                if (confirm(`Sei sicuro di voler eliminare TUTTI gli eventi di questa giornata?`)) {
                    const prefix = isFamily ? 'f-' : '';
                    targetEvents[selectedDateInfo].forEach(evt => {
                        if (evt.amount) {
                            const targetInput = document.getElementById(prefix + evt.category);
                            if (targetInput) {
                                const currentVal = parseFloat(targetInput.value) || 0;
                                targetInput.value = Math.max(0, currentVal - parseFloat(evt.amount));
                            }
                        }
                    });

                    if (isFamily) {
                        await saveFamilyBudget();
                    } else {
                        await saveCurrentMonthBudget();
                        recalculateSummary();
                        initializeLastValues(); 
                    }

                    delete targetEvents[selectedDateInfo];

                    if (isFamily) {
                        localStorage.setItem(`${familyId}_financeEvents`, JSON.stringify(familyEvents));
                        await syncData('family_events', familyEvents, familyId);
                        renderFamilyCalendar();
                        updateRecentMovementsUI();
                    } else {
                        localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
                        await syncData('events', events); 
                        renderCalendar();
                    }

                    if (dayDetailsModal) dayDetailsModal.classList.add('hidden');
                }
            }
        });
    }

    if (closeModalBtn) {
        closeModalBtn.addEventListener('click', () => {
            if (eventModal) eventModal.classList.add('hidden');
        });
    }

    if (saveEventBtn) {
        saveEventBtn.addEventListener('click', async () => {
            if (!eventCategoryInput) return;
            const category = eventCategoryInput.value;
            const amount = eventAmountInput ? parseFloat(eventAmountInput.value.trim()) : 0;
            const familyId = localStorage.getItem(`${currentUser.email}_familyId`);

            if (selectedDateInfo) {
                const isFamily = (accountContext === 'family' && familyId);
                const targetEvents = isFamily ? familyEvents : events;
                
                if (!targetEvents[selectedDateInfo]) {
                    targetEvents[selectedDateInfo] = [];
                }

                const newEvent = { 
                    category, 
                    addedBy: currentUser.username 
                };

                if (!isNaN(amount) && amount > 0) {
                    newEvent.amount = amount;

                    // Aggiorna campo corrispondente nel Budget Corretto
                    const prefix = isFamily ? 'f-' : '';
                    const targetInput = document.getElementById(prefix + category);
                    if (targetInput) {
                        const currentVal = parseFloat(targetInput.value) || 0;
                        targetInput.value = currentVal + amount;
                        
                        if (isFamily) {
                            await saveFamilyBudget();
                        } else {
                            await saveCurrentMonthBudget();
                            recalculateSummary();
                        }
                    }
                }

                targetEvents[selectedDateInfo].push(newEvent);

                // --- FEEDBACK IMMEDIATO UI ---
                if (eventModal) eventModal.classList.add('hidden');

                // Aggiorna Calendario e UI Recente immediatamente
                if (isFamily) {
                    renderFamilyCalendar();
                    updateRecentMovementsUI();
                } else {
                    renderCalendar();
                }

                // Riapri il riepilogo del giorno (la "paginetta" richiesta dall'utente)
                const [year, month, day] = selectedDateInfo.split('-');
                openDayDetailsModal(selectedDateInfo, parseInt(day), parseInt(month) - 1, parseInt(year));

                // --- SALVATAGGIO ASINCRONO (Sincronizzazione) ---
                if (isFamily) {
                    localStorage.setItem(`${familyId}_financeEvents`, JSON.stringify(familyEvents));
                    syncData('family_events', familyEvents, familyId); // Non attendiamo bloccando la UI
                } else {
                    localStorage.setItem(getStorageKey('financeEvents'), JSON.stringify(events));
                    syncData('events', events); // Non attendiamo bloccando la UI
                }

                // Salva il budget se l'importo è cambiato (già aggiornato sopra)
                if (!isNaN(amount) && amount > 0) {
                    if (isFamily) {
                        saveFamilyBudget();
                    } else {
                        saveCurrentMonthBudget();
                        recalculateSummary();
                    }
                }
            }
        });
    }

    // --- Tab Navigation Logic ---
    const allNavBtns = document.querySelectorAll('.nav-btn'); // Now selects both desktop and mobile
    const tabPanes = document.querySelectorAll('.tab-pane');

    function switchTab(tabId) {
        if (!tabId) return;
        
        console.log(`Switching to tab: ${tabId}`);

        // Update Nav UI for all buttons (both desktop and mobile)
        allNavBtns.forEach(btn => {
            btn.classList.toggle('active', btn.getAttribute('data-tab') === tabId);
        });

        // Update Tab Panes
        tabPanes.forEach(pane => {
            if (pane.id === tabId) {
                pane.classList.remove('hidden');
                pane.classList.add('active');
            } else {
                pane.classList.add('hidden');
                pane.classList.remove('active');
            }
        });

        // Update lastActiveTabId if not settings
        if (tabId !== 'tab-settings') {
            lastActiveTabId = tabId;
        }

        // Context Handling
        if (tabId === 'tab-family') {
            accountContext = 'family';
            localStorage.setItem('financeAccountContext', 'family');
            document.body.classList.add('family-mode-active');
            initFamilyUI();
        } else {
            accountContext = 'personal';
            localStorage.setItem('financeAccountContext', 'personal');
            document.body.classList.remove('family-mode-active');
            if (tabId === 'tab-budget') {
                loadCurrentMonthBudget();
            } else if (tabId === 'tab-stats') {
                renderYearlyChart();
            }
        }
    }

    // Attach listeners to all nav-btns
    allNavBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            const targetId = btn.getAttribute('data-tab');
            switchTab(targetId);
        });
    });

    const closeSettingsBtn = document.getElementById('close-settings-btn');
    if (closeSettingsBtn) {
        closeSettingsBtn.addEventListener('click', () => {
            // Simply switch back to the last active tab
            switchTab(lastActiveTabId);
        });
    }

    // --- Yearly Chart Logic ---
    let yearlyChartInstance = null;
    const yearlyTotalIncomeEl = document.getElementById('yearly-total-income');
    const statsYearTitle = document.getElementById('stats-year-title');
    const chartDetailsPanel = document.getElementById('chart-details');
    const noDataMsg = document.getElementById('no-data-msg');

    function renderYearlyChart() {
        const currentYear = currentDate.getFullYear();
        if (statsYearTitle) statsYearTitle.textContent = currentYear;

        let totalIncome = 0;
        const expensesByCategory = {
            'rent': 0,
            'bills': 0,
            'auto': 0,
            'food': 0,
            'subs': 0,
            'leisure': 0,
            'shopping': 0,
            'extra': 0,
            'other': 0
        };

        let hasData = false;

        // 1. Aggrega dai budget mensili salvati (Valori base + eventi che hanno aggiornato i campi)
        for (let m = 1; m <= 12; m++) {
            const key = `${currentYear}-${String(m).padStart(2, '0')}`;
            const budget = monthlyBudgets[key];
            if (budget) {
                totalIncome += (parseFloat(budget.income) || 0);
                expensesByCategory['rent'] += (parseFloat(budget.rent) || 0);
                expensesByCategory['bills'] += (parseFloat(budget.bills) || 0);
                expensesByCategory['auto'] += (parseFloat(budget.auto) || 0);
                expensesByCategory['food'] += (parseFloat(budget.food) || 0);
                expensesByCategory['subs'] += (parseFloat(budget.subs) || 0);
                expensesByCategory['leisure'] += (parseFloat(budget.leisure) || 0);
                expensesByCategory['shopping'] += (parseFloat(budget.shopping) || 0);
                expensesByCategory['extra'] += (parseFloat(budget.extra) || 0);

                if (budget.income || budget.rent || budget.bills || budget.auto || budget.food || budget.subs || budget.leisure || budget.shopping || budget.extra) {
                    hasData = true;
                }
            }
        }

        // 2. Aggrega solo gli eventi "Altro" (quelli che non finiscono nei campi fissi)
        // Nota: Gli eventi come 'bollette' sono già inclusi nei campi sopra perché saveEventBtn aggiorna l'input e salva il budget.
        Object.keys(events).forEach(dateStr => {
            if (dateStr.startsWith(currentYear.toString())) {
                events[dateStr].forEach(evt => {
                    const amount = parseFloat(evt.amount) || 0;
                    if (amount > 0 && evt.category === 'other') {
                        expensesByCategory['other'] += amount;
                        hasData = true;
                    }
                });
            }
        });

        // Mostra stipendio totale anno
        if (yearlyTotalIncomeEl) {
            yearlyTotalIncomeEl.textContent = currency.format(totalIncome);
        }

        const ctx = document.getElementById('yearly-chart');
        if (!ctx) return;

        // Reset details panel
        if (chartDetailsPanel) {
            chartDetailsPanel.innerHTML = `
                <i class="fa-solid fa-hand-pointer" style="font-size: 2rem; color: var(--text-muted); margin-bottom: 1rem;"></i>
                <p style="color: var(--text-muted);">Clicca su una fetta del grafico a torta per visualizzare qui i dettagli della categoria di spesa.</p>
            `;
        }

        // Renderizza elenco risparmi mensili
        renderMonthlySavingsList(currentYear);

        const totalExpenses = Object.values(expensesByCategory).reduce((a, b) => a + b, 0);

        if (!hasData || totalExpenses === 0) {
            if (noDataMsg) noDataMsg.style.display = 'block';
            if (yearlyChartInstance) {
                yearlyChartInstance.destroy();
                yearlyChartInstance = null;
            }
            return;
        } else {
            if (noDataMsg) noDataMsg.style.display = 'none';
        }

        // Prepariamo i dati per Chart.js
        const labels = [];
        const data = [];
        const backgroundColors = [];
        const borderColors = [];

        // Mapping colori per categoria
        const colors = {
            'rent': { bg: 'rgba(239, 68, 68, 0.7)', border: '#ef4444' }, // Red
            'bills': { bg: 'rgba(245, 158, 11, 0.7)', border: '#f59e0b' }, // Orange/Warning
            'auto': { bg: 'rgba(56, 189, 248, 0.7)', border: '#38bdf8' }, // Sky Blue
            'food': { bg: 'rgba(16, 185, 129, 0.7)', border: '#10b981' }, // Emerald / Success
            'subs': { bg: 'rgba(236, 72, 153, 0.7)', border: '#ec4899' }, // Pink
            'leisure': { bg: 'rgba(249, 115, 22, 0.7)', border: '#f97316' }, // Orange
            'shopping': { bg: 'rgba(139, 92, 246, 0.7)', border: '#8b5cf6' }, // Violet
            'extra': { bg: 'rgba(168, 85, 247, 0.7)', border: '#a855f7' }, // Purple
            'other': { bg: 'rgba(148, 163, 184, 0.7)', border: '#94a3b8' } // Slate
        };

        const activeCategories = [];

        Object.keys(expensesByCategory).forEach(cat => {
            if (expensesByCategory[cat] > 0) {
                labels.push(categoryNames[cat]);
                data.push(expensesByCategory[cat]);
                backgroundColors.push(colors[cat].bg);
                borderColors.push(colors[cat].border);
                activeCategories.push(cat); // Teniamo traccia per il click
            }
        });

        if (yearlyChartInstance) {
            yearlyChartInstance.destroy();
        }

        yearlyChartInstance = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: labels,
                datasets: [{
                    label: 'Spese Annuali (€)',
                    data: data,
                    backgroundColor: backgroundColors,
                    borderColor: borderColors,
                    borderWidth: 2,
                    hoverOffset: 10
                }]
            },
            options: {
                responsive: true,
                maintainAspectRatio: false,
                color: '#fff',
                plugins: {
                    legend: {
                        position: window.innerWidth < 768 ? 'bottom' : 'right',
                        labels: {
                            color: '#e2e8f0',
                            font: {
                                family: "'Outfit', sans-serif",
                                size: window.innerWidth < 768 ? 11 : 13
                            },
                            padding: window.innerWidth < 768 ? 10 : 20,
                            boxWidth: window.innerWidth < 768 ? 12 : 20
                        }
                    },
                    tooltip: {
                        callbacks: {
                            label: function (context) {
                                let label = context.label || '';
                                if (label) {
                                    label += ': ';
                                }
                                if (context.parsed !== null) {
                                    label += currency.format(context.parsed);
                                }
                                return label;
                            }
                        }
                    }
                },
                onClick: (event, elements) => {
                    if (elements.length > 0) {
                        const idx = elements[0].index;
                        const categoryName = labels[idx];
                        const amount = data[idx];

                        if (chartDetailsPanel) {
                            chartDetailsPanel.innerHTML = `
                                <h3 style="color: ${borderColors[idx]}; margin-bottom: 0.5rem; font-size: 1.4rem;">${categoryName}</h3>
                                <p style="font-size: 1.1rem;">Hai speso un totale di <strong style="color: white; font-size: 1.5rem;">${currency.format(amount)}</strong> in quest'anno per questa categoria.</p>
                                <p style="font-size: 0.9rem; color: var(--text-muted); margin-top: 10px;">Questa categoria rappresenta il <strong>${((amount / data.reduce((a, b) => a + b, 0)) * 100).toFixed(1)}%</strong> delle tue spese annuali tracciate.</p>
                            `;
                        }
                    }
                }
            }
        });
    }

    function renderMonthlySavingsList(year) {
        const savingsListEl = document.getElementById('monthly-savings-list');
        const yearlyTotalSavingsEl = document.getElementById('yearly-total-savings');
        if (!savingsListEl) return 0;

        savingsListEl.innerHTML = '';
        let totalYearlySavings = 0;

        for (let m = 0; m < 12; m++) {
            const key = `${year}-${String(m + 1).padStart(2, '0')}`;
            const budget = monthlyBudgets[key] || { income: 0, rent: 0, bills: 0, auto: 0, food: 0, subs: 0, leisure: 0, shopping: 0, extra: 0 };

            let monthIncome = parseFloat(budget.income) || 0;
            let monthExpenses = (parseFloat(budget.rent) || 0) +
                (parseFloat(budget.bills) || 0) +
                (parseFloat(budget.auto) || 0) +
                (parseFloat(budget.food) || 0) +
                (parseFloat(budget.subs) || 0) +
                (parseFloat(budget.leisure) || 0) +
                (parseFloat(budget.shopping) || 0) +
                (parseFloat(budget.extra) || 0);

            // Aggiungi solo eventi 'other' (gli altri sono già nel budget sopra)
            Object.keys(events).forEach(dateStr => {
                if (dateStr.startsWith(key)) {
                    events[dateStr].forEach(evt => {
                        const amount = parseFloat(evt.amount) || 0;
                        if (evt.category === 'other') {
                            monthExpenses += amount;
                        }
                    });
                }
            });

            const balance = monthIncome - monthExpenses;
            totalYearlySavings += balance;

            // Crea card per il mese
            const monthCard = document.createElement('div');
            monthCard.className = 'saving-card glass-panel';
            monthCard.style.padding = '1.2rem';
            monthCard.style.textAlign = 'center';
            monthCard.style.borderRadius = '16px';
            monthCard.style.border = '1px solid var(--panel-border)';

            const isPositive = balance >= 0;
            const statusClass = isPositive ? 'success' : 'danger';
            const icon = isPositive ? 'fa-arrow-trend-up' : 'fa-arrow-trend-down';

            monthCard.innerHTML = `
                <span class="month-name" style="display: block; font-weight: 500; margin-bottom: 0.5rem;">${monthNames[m]}</span>
                <div class="balance-value ${statusClass}" style="font-size: 1.3rem; font-weight: 700;">
                    ${currency.format(balance)}
                </div>
                <div style="font-size: 0.8rem; color: var(--text-muted); margin-top: 0.5rem;">
                    <i class="fa-solid ${icon}"></i> Rimanenza
                </div>
            `;

            savingsListEl.appendChild(monthCard);
        }

        // Mostra Risparmio Totale Annuo
        if (yearlyTotalSavingsEl) {
            yearlyTotalSavingsEl.textContent = currency.format(totalYearlySavings);
            yearlyTotalSavingsEl.className = totalYearlySavings >= 0 ? 'success' : 'danger';
        }
    }

    // --- CSV Import/Export Logic ---
    const downloadDemoCsvBtn = document.getElementById('download-demo-csv-btn');

    if (downloadDemoCsvBtn) {
        downloadDemoCsvBtn.addEventListener('click', () => {
            const csvContent =
                "Data,Descrizione,Importo\n" +
                "01/03/2026,Stipendio mensile,2500.00\n" +
                "02/03/2026,Affitto casa mensile,-800.00\n" +
                "05/03/2026,Bolletta Enel Luce,-65.50\n" +
                "08/03/2026,Spesa Supermercato Conad,-120.40\n" +
                "10/03/2026,Rifornimento Benzina Agip,-50.00\n" +
                "12/03/2026,Abbonamento Netflix,-12.99\n" +
                "15/03/2026,Acquisti abbigliamento Zara,-45.90\n" +
                "18/03/2026,Cena Ristorante Pizzeria,-35.00\n" +
                "20/03/2026,Biglietti Cinema,-18.00\n" +
                "25/03/2026,Pagamento Palestra mensile,-40.00\n" +
                "26/03/2026,Spesa imprevista extra,-100.00";

            const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
            const link = document.createElement("a");
            const url = URL.createObjectURL(blob);
            link.setAttribute("href", url);
            link.setAttribute("download", "estratto_conto_demo.csv");
            link.style.visibility = 'hidden';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
        });
    }


    async function processCSV(csvText) {
        const rows = csvText.split('\n').map(row => row.trim()).filter(row => row.length > 0);
        if (rows.length < 2) {
            alert('Il file CSV sembra essere vuoto o non valido.');
            return;
        }

        let importedCategories = {
            income: 0, rent: 0, bills: 0, auto: 0,
            food: 0, subs: 0, leisure: 0, shopping: 0, extra: 0
        };

        let importCount = 0;

        // Saltiamo l'intestazione (i = 1)
        for (let i = 1; i < rows.length; i++) {
            const cols = rows[i].split(',').map(c => c.replace(/(^"|"$)/g, '').trim());

            let desc = "";
            let amount = 0;

            if (cols.length >= 2) {
                for (let j = cols.length - 1; j >= 0; j--) {
                    let valStr = cols[j].replace(/[€"']/g, '').trim();

                    if (valStr.includes(',') && valStr.includes('.')) {
                        if (valStr.lastIndexOf(',') > valStr.lastIndexOf('.')) {
                            valStr = valStr.replace(/\./g, '').replace(',', '.');
                        } else {
                            valStr = valStr.replace(/,/g, '');
                        }
                    } else if (valStr.includes(',')) {
                        valStr = valStr.replace(',', '.');
                    }

                    let val = parseFloat(valStr);
                    if (!isNaN(val)) {
                        amount = val;
                        desc = cols.slice(0, j).join(' ').toLowerCase();
                        break;
                    }
                }
            }

            if (amount !== 0 && desc) {
                const category = categorizeExpense(desc, amount);
                if (category) {
                    importedCategories[category] += Math.abs(amount);
                    importCount++;
                }
            }
        }

        if (importCount > 0) {
            if (importedCategories.income > 0) incomeInput.value = ((parseFloat(incomeInput.value) || 0) + importedCategories.income).toFixed(2);
            if (importedCategories.rent > 0) rentInput.value = ((parseFloat(rentInput.value) || 0) + importedCategories.rent).toFixed(2);
            if (importedCategories.bills > 0) billsInput.value = ((parseFloat(billsInput.value) || 0) + importedCategories.bills).toFixed(2);
            if (importedCategories.auto > 0) autoInput.value = ((parseFloat(autoInput.value) || 0) + importedCategories.auto).toFixed(2);
            if (importedCategories.food > 0) foodInput.value = ((parseFloat(foodInput.value) || 0) + importedCategories.food).toFixed(2);
            if (importedCategories.subs > 0) subsInput.value = ((parseFloat(subsInput.value) || 0) + importedCategories.subs).toFixed(2);
            if (importedCategories.leisure > 0) leisureInput.value = ((parseFloat(leisureInput.value) || 0) + importedCategories.leisure).toFixed(2);
            if (importedCategories.shopping > 0) shoppingInput.value = ((parseFloat(shoppingInput.value) || 0) + importedCategories.shopping).toFixed(2);
            if (importedCategories.extra > 0) extraInput.value = ((parseFloat(extraInput.value) || 0) + importedCategories.extra).toFixed(2);

            await saveCurrentMonthBudget();
            recalculateSummary();
            initializeLastValues();
            alert(`Estratto conto importato con successo!\nSono stati elaborati ${importCount} movimenti e suddivisi automaticamente nelle categorie del budget.`);
        } else {
            alert('Nessun movimento valido o riconosciuto trovato nel CSV. Assicurati che contenga descrizioni e importi validi (separati da virgola).');
        }
    }


    // --- Account Context Switching Logic ---
    const contextSwitcher = document.getElementById('context-switcher');
    const switchPersonalBtn = document.getElementById('switch-personal-btn');
    const switchFamilyBtn = document.getElementById('switch-family-btn');

    async function switchAccountContext(context) {
        if (!currentUser) return;
        
        accountContext = context;
        localStorage.setItem('financeAccountContext', context);
        
        // Aggiorna UI Pulsanti
        if (switchPersonalBtn && switchFamilyBtn) {
            if (context === 'personal') {
                switchPersonalBtn.classList.add('active');
                switchFamilyBtn.classList.remove('active');
                document.body.classList.remove('family-mode-active');
            } else {
                switchPersonalBtn.classList.remove('active');
                switchFamilyBtn.classList.add('active');
                document.body.classList.add('family-mode-active');
            }
        }

        // Forza ricarica dati per il nuovo contesto
        monthlyBudgets = {};
        events = {};
        await loadAllData();
        
        // Torna alla home per sicurezza o aggiorna tab corrente
        const activeTab = document.querySelector('.nav-btn.active');
        if (activeTab) activeTab.click();
    }

    if (switchPersonalBtn) {
        switchPersonalBtn.addEventListener('click', () => switchAccountContext('personal'));
    }

    if (switchFamilyBtn) {
        switchFamilyBtn.addEventListener('click', () => {
            const familyId = localStorage.getItem(`${currentUser.email}_familyId`);
            if (!familyId) {
                alert("Non sei ancora in un gruppo familiare. Crea o unisciti a un gruppo nella sezione 'Famiglia'.");
                // Opzionale: porta l'utente alla tab famiglia
                const familyTabBtn = document.querySelector('[data-tab="tab-family"]');
                if (familyTabBtn) familyTabBtn.click();
                return;
            }
            switchAccountContext('family');
        });
    }

    // --- Family Workspace Logic (Restylized) ---
    const createFamilyBtnMain = document.getElementById('create-family-btn-main');
    const joinFamilyBtnMain = document.getElementById('join-family-btn-main');
    const joinFamilyCodeInputMain = document.getElementById('join-family-code-main');
    const leaveFamilyBtnMain = document.getElementById('leave-family-btn-main');
    const copyFamilyCodeBtnMain = document.getElementById('copy-family-code-main');
    const familyInactiveViewMain = document.getElementById('family-inactive-view-main');
    const familyActiveViewMain = document.getElementById('family-active-view-main');
    const displayFamilyCodeMain = document.getElementById('display-family-code-main');
    const familyMembersListMain = document.getElementById('family-members-list-main');

    // --- Family Dashboard Logic ---
    const fIncomeInput = document.getElementById('f-income');
    const fRentInput = document.getElementById('f-rent');
    const fBillsInput = document.getElementById('f-bills');
    const fAutoInput = document.getElementById('f-auto');
    const fFoodInput = document.getElementById('f-food');
    const fSubsInput = document.getElementById('f-subs');
    const fLeisureInput = document.getElementById('f-leisure');
    const fShoppingInput = document.getElementById('f-shopping');
    const fExtraInput = document.getElementById('f-extra');
    const fSummaryIncome = document.getElementById('f-summary-income');
    const fSummaryExpenses = document.getElementById('f-summary-expenses');
    const fSummaryRemaining = document.getElementById('f-summary-remaining');
    const fCalendarGrid = document.getElementById('f-calendar-grid');
    const fRecentMovementsList = document.getElementById('f-recent-movements-list');
    const fAllBudgetInputs = [fIncomeInput, fRentInput, fBillsInput, fAutoInput, fFoodInput, fSubsInput, fLeisureInput, fShoppingInput, fExtraInput];
    let currentFamilyDate = new Date();

    async function initFamilyUI() {
        if (!currentUser) return;
        const familyId = localStorage.getItem(`${currentUser.email}_familyId`);
        const transferSection = document.getElementById('transfer-to-family-section');
        
        if (familyId) {
            if (familyInactiveViewMain) familyInactiveViewMain.classList.add('hidden');
            if (familyActiveViewMain) familyActiveViewMain.classList.remove('hidden');
            if (displayFamilyCodeMain) displayFamilyCodeMain.textContent = familyId;
            if (transferSection) transferSection.classList.remove('hidden');
            
            // Load Family UI Data
            loadFamilyBudget();
            renderFamilyCalendar();
            updateFamilyMembersUI(familyId);
            updateRecentMovementsUI();
        } else {
            if (familyInactiveViewMain) familyInactiveViewMain.classList.remove('hidden');
            if (familyActiveViewMain) familyActiveViewMain.classList.add('hidden');
            if (transferSection) transferSection.classList.add('hidden');
        }
    }

    function loadFamilyBudget() {
        const date = currentFamilyDate;
        const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
        const budget = familyMonthlyBudgets[monthKey] || {};

        if (fIncomeInput) fIncomeInput.value = budget.income || '';
        if (fRentInput) fRentInput.value = budget.rent || '';
        if (fBillsInput) fBillsInput.value = budget.bills || '';
        if (fAutoInput) fAutoInput.value = budget.auto || '';
        if (fFoodInput) fFoodInput.value = budget.food || '';
        if (fSubsInput) fSubsInput.value = budget.subs || '';
        if (fLeisureInput) fLeisureInput.value = budget.leisure || '';
        if (fShoppingInput) fShoppingInput.value = budget.shopping || '';
        if (fExtraInput) fExtraInput.value = budget.extra || '';

        recalculateFamilySummary();
    }

    async function saveFamilyBudget() {
        const familyId = localStorage.getItem(`${currentUser.email}_familyId`);
        if (!familyId) return;

        const date = currentFamilyDate;
        const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
        
        familyMonthlyBudgets[monthKey] = {
            income: parseFloat(fIncomeInput.value) || 0,
            rent: parseFloat(fRentInput.value) || 0,
            bills: parseFloat(fBillsInput.value) || 0,
            auto: parseFloat(fAutoInput.value) || 0,
            food: parseFloat(fFoodInput.value) || 0,
            subs: parseFloat(fSubsInput.value) || 0,
            leisure: parseFloat(fLeisureInput.value) || 0,
            shopping: parseFloat(fShoppingInput.value) || 0,
            extra: parseFloat(fExtraInput.value) || 0
        };

        localStorage.setItem(`${familyId}_financeMonthlyBudgets`, JSON.stringify(familyMonthlyBudgets));
        await syncData('family_budgets', familyMonthlyBudgets, familyId);
        recalculateFamilySummary();
    }

    function recalculateFamilySummary() {
        const income = parseFloat(fIncomeInput.value) || 0;
        const rent = parseFloat(fRentInput.value) || 0;
        const bills = parseFloat(fBillsInput.value) || 0;
        const auto = parseFloat(fAutoInput.value) || 0;
        const food = parseFloat(fFoodInput.value) || 0;
        const subs = parseFloat(fSubsInput.value) || 0;
        const leisure = parseFloat(fLeisureInput.value) || 0;
        const shopping = parseFloat(fShoppingInput.value) || 0;
        const extra = parseFloat(fExtraInput.value) || 0;

        const totalExpenses = rent + bills + auto + food + subs + leisure + shopping + extra;
        const remaining = income - totalExpenses;

        if (fSummaryIncome) fSummaryIncome.textContent = currency.format(income);
        if (fSummaryExpenses) fSummaryExpenses.textContent = currency.format(totalExpenses);
        if (fSummaryRemaining) fSummaryRemaining.textContent = currency.format(remaining);
    }

    function renderFamilyCalendar() {
        if (!fCalendarGrid) return;
        fCalendarGrid.innerHTML = '';
        const date = currentFamilyDate;
        
        const fCalendarMonthYear = document.getElementById('f-calendar-month-year');
        if (fCalendarMonthYear) {
            const monthNames = [
                getI18n('jan'), getI18n('feb'), getI18n('mar'), getI18n('apr'), getI18n('may'), getI18n('jun'),
                getI18n('jul'), getI18n('aug'), getI18n('sep'), getI18n('oct'), getI18n('nov'), getI18n('dec')
            ];
            // If the above keys are missing in translations, use default Italian for now or add them
            fCalendarMonthYear.textContent = `${monthNames[date.getMonth()] || date.toLocaleString(currentLanguage, { month: 'long' })} ${date.getFullYear()}`;
        }
        
        const daysInMonth = new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate();
        const startDay = new Date(date.getFullYear(), date.getMonth(), 1).getDay();
        const adjustedStart = startDay === 0 ? 6 : startDay - 1;

        for (let i = 0; i < adjustedStart; i++) {
            const empty = document.createElement('div');
            empty.className = 'calendar-day empty';
            fCalendarGrid.appendChild(empty);
        }

        const today = new Date();

        for (let d = 1; d <= daysInMonth; d++) {
            const dayEl = document.createElement('div');
            dayEl.className = 'calendar-day';
            const dateStr = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
            const dayEvents = familyEvents[dateStr] || [];

            if (date.getFullYear() === today.getFullYear() && date.getMonth() === today.getMonth() && d === today.getDate()) {
                dayEl.classList.add('today');
            }

            const dateNum = document.createElement('span');
            dateNum.className = 'date-num';
            dateNum.textContent = d;
            dayEl.appendChild(dateNum);
            
            if (dayEvents.length > 0) {
                dayEvents.forEach((evt, index) => {
                    const evtDiv = document.createElement('div');
                    evtDiv.className = `event-indicator event-${categoryToType[evt.category] || 'other'}`;

                    let amountStr = '';
                    if (evt.amount) {
                        const type = categoryToType[evt.category] || 'other';
                        const prefix = type === 'expense' ? '-' : (type === 'income' ? '+' : '');
                        amountStr = ` ${prefix}€${evt.amount}`;
                    }

                    let titleText = getCategoryName(evt.category) || 'Altro';
                    if (evt.addedBy) {
                        titleText = `${titleText}`;
                    }
                    evtDiv.textContent = `${titleText}${amountStr}`;

                    evtDiv.addEventListener('click', (e) => {
                        e.stopPropagation();
                        openDayDetailsModal(dateStr, d, date.getMonth(), date.getFullYear());
                    });

                    dayEl.appendChild(evtDiv);
                });
            }
            
            dayEl.addEventListener('click', () => {
                openDayDetailsModal(dateStr, d, date.getMonth(), date.getFullYear());
            });

            fCalendarGrid.appendChild(dayEl);
        }
    }

    // Attach Listeners for Family Inputs
    fAllBudgetInputs.forEach(inp => {
        if (inp) {
            inp.addEventListener('input', recalculateFamilySummary);
            inp.addEventListener('change', async () => {
                const enteredVal = parseFloat(inp.value) || 0;
                inp.value = enteredVal;

                if (enteredVal !== 0) {
                    const today = new Date();
                    const dateStr = `${currentFamilyDate.getFullYear()}-${String(currentFamilyDate.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;

                    if (!familyEvents[dateStr]) familyEvents[dateStr] = [];

                    const rawCategory = inp.id.replace('f-', ''); 

                    familyEvents[dateStr].push({
                        category: rawCategory,
                        amount: Math.abs(enteredVal),
                        addedBy: currentUser ? currentUser.username : 'Sconosciuto'
                    });

                    const familyId = localStorage.getItem(`${currentUser.email}_familyId`);
                    if (familyId) {
                        localStorage.setItem(`${familyId}_financeEvents`, JSON.stringify(familyEvents));
                        if (supabase && typeof syncData === 'function') await syncData('family_events', familyEvents, familyId);
                    }
                    renderFamilyCalendar();
                    updateRecentMovementsUI();
                }
                await saveFamilyBudget();
            });
        }
    });

    const fPrevMonthBtn = document.getElementById('f-prev-month');
    const fNextMonthBtn = document.getElementById('f-next-month');

    if (fPrevMonthBtn) {
        fPrevMonthBtn.addEventListener('click', async () => {
            await saveFamilyBudget();
            currentFamilyDate.setMonth(currentFamilyDate.getMonth() - 1);
            loadFamilyBudget();
            renderFamilyCalendar();
        });
    }

    if (fNextMonthBtn) {
        fNextMonthBtn.addEventListener('click', async () => {
            await saveFamilyBudget();
            currentFamilyDate.setMonth(currentFamilyDate.getMonth() + 1);
            loadFamilyBudget();
            renderFamilyCalendar();
        });
    }

    function updateRecentMovementsUI() {
        if (!fRecentMovementsList) return;
        fRecentMovementsList.innerHTML = '';
        
        let allMovements = [];
        Object.keys(familyEvents).forEach(date => {
            familyEvents[date].forEach(evt => {
                allMovements.push({ ...evt, date });
            });
        });

        // Ordina per data decrescente
        allMovements.sort((a, b) => new Date(b.date) - new Date(a.date));

        // Mostra gli ultimi 10
        const recent = allMovements.slice(0, 10);
        
        if (recent.length === 0) {
            fRecentMovementsList.innerHTML = '<p style="color: var(--text-muted); font-size: 0.9rem;">Nessun movimento recente.</p>';
            return;
        }

        recent.forEach(m => {
            const item = document.createElement('div');
            item.className = 'movement-item';
            item.innerHTML = `
                <div class="movement-info">
                    <span style="font-size: 0.9rem; font-weight: 600;">${getCategoryName(m.category) || m.category}</span>
                    <span class="movement-author"><i class="fa-solid fa-user-pen"></i> di ${m.addedBy || 'Utente'}</span>
                </div>
                <div style="text-align: right;">
                    <div class="movement-amount">${currency.format(m.amount || 0)}</div>
                    <div style="font-size: 0.7rem; color: var(--text-muted);">${m.date}</div>
                </div>
            `;
            fRecentMovementsList.appendChild(item);
        });
    }

        async function syncFamilyMembers(familyId, action, isCreator = false) {
            if (!supabase || !currentUser || !familyId) return;
            
            try {
                // Fetch the MOST UP TO DATE members from the family creator's record
                // The creator is the one whose user_id is the familyId
                const { data, error } = await supabase
                    .from('user_data')
                    .select('value')
                    .eq('user_id', familyId)
                    .eq('key', 'members')
                    .maybeSingle();
                
                let members = [];
                if (data && data.value) {
                    if (Array.isArray(data.value)) {
                        members = data.value;
                    } else if (typeof data.value === 'string') {
                        try {
                            const parsed = JSON.parse(data.value);
                            if (Array.isArray(parsed)) members = parsed;
                        } catch(e) { }
                    }
                }
                
                if (action === 'add') {
                    if (members.length === 0) isCreator = true; // Prima persona è il creatore

                    // Check if already in list
                    const exists = members.find(m => m.email === currentUser.email);
                    if (!exists) {
                        members.push({
                            email: currentUser.email,
                            username: currentUser.username || 'Utente',
                            role: isCreator ? 'creatore' : 'membro',
                            joined_at: new Date().toISOString()
                        });
                    } else {
                        // Update username just in case it changed
                        exists.username = currentUser.username || 'Utente';
                        if (isCreator) exists.role = 'creatore';
                    }
                } else if (action === 'remove') {
                    const targetEmail = isCreator || currentUser.email; // Se isCreator contiene l'email vera da rimuovere, usala
                    members = members.filter(m => m.email !== targetEmail);
                }
                
                // Directly update members field for the familyId (the creator)
                const { error: upsertError } = await supabase
                    .from('user_data')
                    .upsert({
                        user_id: familyId,
                        key: 'members',
                        value: members,
                        updated_at: new Date().toISOString()
                    }, { onConflict: 'user_id,key' });

                if (upsertError) throw upsertError;
                
                // Also update local cache for immediate UI responsiveness
                localStorage.setItem(`${familyId}_members`, JSON.stringify(members));
                
                // Refresh the UI if we are in the family view
                if (typeof updateFamilyMembersUI === 'function') {
                    updateFamilyMembersUI(familyId);
                }
            } catch (err) {
                console.error("Error syncing family members:", err);
            }
        }

        async function updateFamilyMembersUI(familyId) {
            if (!familyMembersListMain || !familyId) return;
            
            let members = [];
            const savedMembers = localStorage.getItem(`${familyId}_members`);
            if (savedMembers) {
                try {
                    members = JSON.parse(savedMembers);
                } catch(e) { }
            }

            const renderMembersList = (membersList) => {
                familyMembersListMain.innerHTML = '';
                if (membersList && membersList.length > 0) {
                    const amICreator = currentUser && currentUser.id === familyId;
                    
                    membersList.forEach(m => {
                        const mEl = document.createElement('div');
                        mEl.className = 'member-card';
                        mEl.style.display = 'flex';
                        mEl.style.flexDirection = 'column';
                        mEl.style.alignItems = 'center';
                        mEl.style.textAlign = 'center';
                        mEl.style.gap = '10px';
                        mEl.style.padding = '1.2rem';
                        mEl.style.background = 'rgba(255,255,255,0.05)';
                        mEl.style.borderRadius = '20px';
                        mEl.style.border = '1px solid var(--panel-border)';
                        mEl.style.transition = 'transform 0.2s';
                        mEl.style.position = 'relative'; // Per il bottoncino elimina
                        
                        let kickButtonHtml = '';
                        if (amICreator && m.email !== currentUser.email) {
                            kickButtonHtml = `
                                <button class="kick-member-btn" data-email="${m.email}" data-name="${m.username}" style="position: absolute; top: 10px; right: 10px; background: rgba(220, 38, 38, 0.1); color: var(--danger); border: none; border-radius: 50%; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s;">
                                    <i class="fa-solid fa-xmark"></i>
                                </button>
                            `;
                        }

                        mEl.innerHTML = `
                            ${kickButtonHtml}
                            <div class="family-member-avatar" style="width: 50px; height: 50px; background: var(--success); border-radius: 15px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 1.2rem; color: white; box-shadow: 0 5px 15px rgba(16, 185, 129, 0.3);">
                                ${(m.username || m.email).charAt(0).toUpperCase()}
                            </div>
                            <div style="overflow: hidden; width: 100%;">
                                <div style="font-size: 0.95rem; font-weight: 700; color: var(--text-main); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                    ${m.username || 'Utente'}
                                </div>
                                <div style="font-size: 0.75rem; color: var(--text-muted); opacity: 0.7; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                    ${m.email === currentUser.email ? 'Tu' : m.email}
                                </div>
                                ${m.role === 'creatore' ? '<div style="font-size: 0.75rem; color: var(--primary); font-weight: bold; margin-top: 4px;"><i class="fa-solid fa-crown"></i> Creatore</div>' : '<div style="font-size: 0.75rem; color: var(--text-muted); margin-top: 4px;">Membro</div>'}
                            </div>
                        `;
                        familyMembersListMain.appendChild(mEl);
                    });

                    // Aggiungi Event Listeners per espellere membri
                    if (amICreator) {
                        const kickBtns = familyMembersListMain.querySelectorAll('.kick-member-btn');
                        kickBtns.forEach(btn => {
                            btn.addEventListener('click', async (e) => {
                                const emailToKick = e.currentTarget.getAttribute('data-email');
                                const nameToKick = e.currentTarget.getAttribute('data-name');
                                if (confirm(`Sei sicuro di voler espellere ${nameToKick} dal tuo Workspace?\nL'utente non vedrà più i dati della tua famiglia.`)) {
                                    e.currentTarget.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
                                    e.currentTarget.disabled = true;
                                    await syncFamilyMembers(familyId, 'remove', emailToKick);
                                    // Non serve ricaricare UI, syncFamilyMembers chiama già check e render
                                }
                            });
                        });
                    }
                } else {
                    familyMembersListMain.innerHTML = `
                        <div style="grid-column: 1 / -1; padding: 2.5rem; text-align: center; background: var(--panel-bg); border-radius: 15px; border: 1px dashed var(--panel-border); color: var(--text-main);">
                            <i class="fa-solid fa-user-clock" style="font-size: 2rem; margin-bottom: 1rem; opacity: 0.5;"></i>
                            <p style="font-size: 0.9rem;">Caricamento membri...</p>
                        </div>
                    `;
                }
            };

            // Mostra i membri dalla cache locale immediatamente (se ci sono)
            renderMembersList(members);

            // Fetch sempre asincrono per l'ultima versione dei membri
            if (supabase) {
                try {
                    // Controlla il creatore della famiglia
                    const { data } = await supabase
                        .from('user_data')
                        .select('value')
                        .eq('user_id', familyId)
                        .eq('key', 'members')
                        .maybeSingle();

                    if (data && data.value) {
                        let fetchedMembers = data.value;
                        if (typeof fetchedMembers === 'string') {
                            try { fetchedMembers = JSON.parse(fetchedMembers); } catch(e){}
                        }
                        if (Array.isArray(fetchedMembers)) {
                            // CONTROLLO ESPULSIONE: L'utente corrente non è più nella lista scaricata dal server?
                            const amICreator = currentUser && currentUser.id === familyId;
                            const stillInFamily = fetchedMembers.some(m => m.email === currentUser.email);
                            
                            if (!amICreator && !stillInFamily) {
                                // Siamo stati espulsi!
                                console.warn("WORKSPACE EXPULSION DETECTED");
                                localStorage.removeItem(`${currentUser.email}_familyId`);
                                
                                // Set a session lock to prevent loadUserData from restoring it immediately on reload
                                sessionStorage.setItem('prevent_family_restore', 'true');
                                
                                // Sincronizza la rimozione sul cloud
                                await syncData('family_id', null);
                                
                                alert("Sei stato rimosso da questo Workspace Familiare dal Creatore.");
                                location.reload();
                                return;
                            }

                            localStorage.setItem(`${familyId}_members`, JSON.stringify(fetchedMembers));
                            // Aggiorna l'interfaccia solo se ci sono discrepanze
                            if (JSON.stringify(members) !== JSON.stringify(fetchedMembers)) {
                                members = fetchedMembers;
                                renderMembersList(members);
                            }
                        }
                    }
                } catch (err) {
                    console.error("Fetch members failure:", err);
                }
            }
        }

    if (createFamilyBtnMain) {
        createFamilyBtnMain.addEventListener('click', async () => {
            if (!currentUser || !currentUser.id) return alert("Errore: devi effettuare l'accesso per creare un Workspace.");
            const newFamilyId = currentUser.id;
            if (confirm(`Vuoi creare un nuovo Workspace Familiare?\nVerrai impostato come Creatore. Il codice da condividere sarà il tuo ID Account.`)) {
                localStorage.setItem(`${currentUser.email}_familyId`, newFamilyId);
                if (supabase) {
                    await syncData('family_id', newFamilyId);
                    await syncFamilyMembers(newFamilyId, 'add', true);
                }
                setTimeout(async () => {
                    await loadUserData(); // Refresh data from cloud
                    initFamilyUI();
                }, 500);
                alert("Workspace creato! Ora copia il codice qui sotto e condividilo con i membri della tua famiglia.");
            }
        });
    }

    if (joinFamilyBtnMain) {
        joinFamilyBtnMain.addEventListener('click', async () => {
            const code = joinFamilyCodeInputMain.value.trim();
            if (code.length < 10) {
                alert("Inserisci un codice valido.");
                return;
            }
            if (code.startsWith('FAM-')) {
                alert("Questo formato di codice è obsoleto. Chiedi al Creatore di fornirti il suo nuovo codice ID Account (36 caratteri).");
                return;
            }
            localStorage.setItem(`${currentUser.email}_familyId`, code);
            if (supabase) {
                await syncData('family_id', code);
                await syncFamilyMembers(code, 'add');
            }
            setTimeout(async () => {
                await loadUserData(); // Refresh data from cloud
                initFamilyUI();
            }, 500);
            alert(`Ti sei unito al Workspace! I dati sono stati sincronizzati con successo.`);
        });
    }

    if (leaveFamilyBtnMain) {
        leaveFamilyBtnMain.addEventListener('click', async () => {
            if (confirm("Sei sicuro di voler uscire dal Workspace Familiare? Tornerai al tuo budget personale e il codice verrà rimosso dal tuo profilo.")) {
                const oldFamilyId = localStorage.getItem(`${currentUser.email}_familyId`);
                if (supabase && oldFamilyId) {
                    await syncFamilyMembers(oldFamilyId, 'remove');
                }
                localStorage.removeItem(`${currentUser.email}_familyId`);
                if (supabase) await syncData('family_id', null);
                
                setTimeout(async () => {
                    await loadUserData();
                    initFamilyUI();
                }, 500);
            }
        });
    }

    if (copyFamilyCodeBtnMain) {
        copyFamilyCodeBtnMain.addEventListener('click', () => {
            const code = displayFamilyCodeMain.textContent;
            navigator.clipboard.writeText(code).then(() => {
                const icon = copyFamilyCodeBtnMain.querySelector('i');
                icon.className = 'fa-solid fa-check';
                setTimeout(() => icon.className = 'fa-solid fa-copy', 2000);
            });
        });
    }

    const confirmTransferBtn = document.getElementById('confirm-transfer-btn');
    const transferAmountInput = document.getElementById('transfer-amount');

    if (confirmTransferBtn) {
        confirmTransferBtn.addEventListener('click', async () => {
            const amount = parseFloat(transferAmountInput.value) || 0;
            if (amount <= 0) {
                alert("Inserisci un importo valido da trasferire.");
                return;
            }

            const familyId = localStorage.getItem(`${currentUser.email}_familyId`);
            if (!familyId) return;

            if (confirm(`Sei sicuro di voler trasferire ${currency.format(amount)} al budget di famiglia?\nL'importo verrà aggiunto come entrata nel budget comune e segnato come spesa extra nel tuo budget personale.`)) {
                try {
                    // 1. Aggiungi al budget famiglia (come income)
                    const date = new Date();
                    const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
                    const dayKey = `${monthKey}-${String(date.getDate()).padStart(2, '0')}`;

                    if (!familyMonthlyBudgets[monthKey]) familyMonthlyBudgets[monthKey] = { income: 0, rent: 0, bills: 0, auto: 0, food: 0, subs: 0, leisure: 0, shopping: 0, extra: 0 };
                    familyMonthlyBudgets[monthKey].income = (parseFloat(familyMonthlyBudgets[monthKey].income) || 0) + amount;

                    // Aggiungi evento famiglia
                    if (!familyEvents[dayKey]) familyEvents[dayKey] = [];
                    familyEvents[dayKey].push({
                        category: 'income',
                        amount: amount,
                        description: `Trasferimento da ${currentUser.username}`,
                        addedBy: currentUser.username
                    });

                    // Salva famiglia
                    localStorage.setItem(`${familyId}_financeMonthlyBudgets`, JSON.stringify(familyMonthlyBudgets));
                    localStorage.setItem(`${familyId}_financeEvents`, JSON.stringify(familyEvents));
                    if (supabase) {
                        await syncData('family_budgets', familyMonthlyBudgets, familyId);
                        await syncData('family_events', familyEvents, familyId);
                    }

                    // 2. Sottrai dal budget personale (come extra o nuova categoria)
                    // Per semplicità lo mettiamo in 'extra'
                    const extraInput = document.getElementById('extra');
                    if (extraInput) {
                        extraInput.value = (parseFloat(extraInput.value) || 0) + amount;
                        // Trigger recalculation and save for personal
                        recalculateSummary();
                        saveCurrentMonthBudget(); 
                    }

                    // Reset input
                    transferAmountInput.value = '';
                    alert("Trasferimento completato con successo!");
                    
                    // Se siamo nella tab famiglia, aggiorna
                    if (document.body.classList.contains('family-mode-active')) {
                        loadFamilyBudget();
                        renderFamilyCalendar();
                        updateRecentMovementsUI();
                    }
                } catch (err) {
                    console.error("Errore durante il trasferimento:", err);
                    alert("Si è verificato un errore durante il trasferimento.");
                }
            }
        });
    }

    async function loadAllData() {
        // Reset local state
        monthlyBudgets = {};
        events = {};
        
        // Re-load from storage (which now uses the new getStorageKey)
        const savedBudgets = localStorage.getItem(getStorageKey('monthlyBudgets'));
        if (savedBudgets) monthlyBudgets = JSON.parse(savedBudgets);
        
        const savedEvents = localStorage.getItem(getStorageKey('financeEvents'));
        if (savedEvents) events = JSON.parse(savedEvents);

        // Se c'è Supabase, prova a tirare giù i dati condivisi
        if (supabase && currentUser) {
            const sharedKey = getStorageKey('budgets'); // o events, goals
            // Qui andrebbe la logica di sync bidirezionale se implementata
        }

        recalculateSummary();
        renderCalendar();
        loadGoals();
        initializeLastValues();

        // Load specific user preferences like card style (Disabilitato per richiesta "sempre blu")
        // const savedCardStyle = localStorage.getItem(getStorageKey('financeCardStyle')) || 'card-style-default';
        // if (typeof window.setCardStyle === 'function') window.setCardStyle(savedCardStyle);
    }

    // Inizializza lo stato dell'app
    checkAuthStatus();
    // NOTA: initFamilyUI viene rimosso qui perché viene chiamato internamente da loadUserData
    // al termine della sincronizzazione dei dati, evitando race conditions.
});

